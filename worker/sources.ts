...
function pickSections<T extends { line: string }>(sections: T[], keywords: string[]): T[] {
  const scored = sections.map((s, i) => ({ s, i, hits: keywords.filter((k) => s.line.toLowerCase().includes(k.toLowerCase())).length }));
...
export async function fetchWikipedia(lang: string, title: string, keywords: string[], fetchImpl: typeof fetch = fetch) {
  ...
  const sections = await Promise.all(picked.map(async (sec) => {
    const wt = await apiFetch(`${base}?action=parse&page=${t}&section=${sec.index}&prop=wikitext&${common}`, fetchImpl);
...
export async function lookupPerson(label: string, startDate: string, fetchImpl: typeof fetch = fetch) {
  const startYear = isoYear(startDate);
  const search = await apiFetch(`https://www.wikidata.org/w/api.php?action=wbsearchentities&search=${encodeURIComponent(label)}&language=en&type=item&format=json&origin=*`, fetchImpl);
  for (const hit of (search.search ?? []).slice(0, 5)) {
    const ent = await apiFetch(`https://www.wikidata.org/w/api.php?action=wbgetentities&ids=${hit.id}&props=claims&format=json&origin=*`, fetchImpl);
...
export async function lookupParty(label: string, fetchImpl: typeof fetch = fetch) {
  ...
  const seatsAmount = claims.P1410?.[0]?.mainsnak?.datavalue?.value?.amount ?? claims.P1342?.[0]?.mainsnak?.datavalue?.value?.amount;
  const seats = seatsAmount !== undefined ? parseInt(String(seatsAmount).replace("+", ""), 10) : undefined;
