// The theme's data and colour maths without zod, so the landing chunk can read the default theme and fit colours
// without the schema library: the default tokens, WCAG contrast and the OKLCH fix to 4.5:1. worker/tokens.ts holds
// the schemas and re-exports these for the worker.
...
export function fitContrast(
  ...
  if (![foreground, ...backgrounds].every((colour) => HEX.test(colour))) return null;
