...
export function parseRow(row: any): ScenarioRow {
  ...
  if (!parsed.success)
    return {
      ...
      fragments,
      status: "failed",
      error: "pack no longer matches the schema",
    };
...
export async function putStatus(env: Env, id: string, step: string, fragment?: string) {
  if (fragment) {
    await env.DB.prepare(
      ...
      .bind(step, fragment, id)
      .run();
  } else {
    await env.DB.prepare("UPDATE scenarios SET status = 'building', step = ? WHERE id = ?")
      .bind(step, id)
      .run();
  }
}
...
export async function newScenario(env: Env, id: string, prompt: string) {
  await env.DB.prepare(
    "INSERT INTO scenarios (id, status, step, prompt, fragments, created) VALUES (?, 'queued', 'plan', ?, '[]', ?)",
  )
    .bind(id, prompt, Date.now())
    .run();
}
...
export async function putMeta(
  env: Env,
  id: string,
  m: { lang: string; title: string; era: string; place: string; description: string },
) {
  await env.DB.prepare(
    "UPDATE scenarios SET lang = ?, title = ?, era = ?, place = ?, description = ? WHERE id = ?",
  )
    .bind(m.lang, m.title, m.era, m.place, m.description, id)
    .run();
}

export async function failScenario(env: Env, id: string, message: string) {
  await env.DB.prepare("UPDATE scenarios SET status = 'failed', error = ? WHERE id = ?")
    .bind(message, id)
    .run();
}
...
export type DailyMeta = DailyRow & {
  title: string | null;
  era: string | null;
  place: string | null;
};
...
export const shiftDay = (day: string, by: number) =>
  dayKey(Date.parse(`${day}T00:00:00Z`) + by * 86_400_000);
...
export async function takeDaily(env: Env, day: string): Promise<boolean> {
  const r = await env.DB.prepare(
    "INSERT INTO dailies (day, status, created) VALUES (?, 'building', ?) ON CONFLICT(day) DO NOTHING",
  )
    .bind(day, Date.now())
    .run();
...
export async function putDaily(
  ...
  await env.DB.prepare("UPDATE dailies SET prompt = ?, scenario = ?, status = ? WHERE day = ?")
    .bind(prompt, scenario, status, day)
    .run();
}
...
export async function takeAttempt(
  ...
  const r = await env.DB.prepare(
    "INSERT INTO daily_plays (id, day, game, created) VALUES (?, ?, ?, ?) ON CONFLICT(id, day) DO NOTHING",
  )
    .bind(id, day, game, Date.now())
    .run();
  return (r.meta.changes ?? 0) > 0;
}
...
export async function dropAttempt(env: Env, id: string, day: string) {
  await env.DB.prepare("DELETE FROM daily_plays WHERE id = ? AND day = ? AND ended = 0")
    .bind(id, day)
    .run();
}

export function getPlay(env: Env, id: string, day: string): Promise<PlayRow | null> {
  return env.DB.prepare("SELECT * FROM daily_plays WHERE id = ? AND day = ?")
    .bind(id, day)
    .first<PlayRow>();
}
...
export async function playCount(env: Env, day: string): Promise<number> {
  const row = await env.DB.prepare(
    "SELECT COUNT(*) AS n FROM daily_plays WHERE day = ? AND ended = 1",
  )
    .bind(day)
    .first<{ n: number }>();
  return row?.n ?? 0;
...
export async function streakOf(
  ...
  const { results } = await env.DB.prepare(
    "SELECT day FROM daily_plays WHERE id = ? AND ended = 1 ORDER BY day DESC LIMIT ?",
  )
    .bind(id, lookback)
    .all<{ day: string }>();
...
export class BuildsDO extends DurableObject<Env> {
  ...
  takeGame(): Promise<boolean> {
    return this.count("games", Number(this.env.DAILY_GAME_CAP));
  }
