...
const fakeDB = (days: string[]) =>
  ({
    prepare: () => ({
      bind: () => ({ all: async () => ({ results: days.map((day) => ({ day })) }) }),
    }),
...
test("the streak counts back from today, allows today unplayed, and stops at the first gap", async () => {
  ...
  expect(
    await streakOf(env(["2026-09-22", "2026-09-21", "2026-09-20"]), "p", "2026-09-22", 60),
  ).toBe(3);
...
const steps = () => {
  ...
  return {
    names,
    step: {
      do: async (name: string, _c: unknown, fn?: () => unknown) => {
        names.push(name);
        return (fn ?? (_c as () => unknown))();
      },
