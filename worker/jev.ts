...
export type Env = {
  ...
  BUILD: Workflow;
  DAILY: Workflow;
  BUILDS: DurableObjectNamespace<import("./db").BuildsDO>;
...
export type Answers = Record<
  string,
  { noul?: number; score?: number; probabilities?: Record<string, number> }
>;
...
): Promise<{ answers: Answers; usage: { input_tokens: number; cost?: number } }> {
...
export const nouls = (answers: Answers, prefix: string): Record<string, number> =>
  Object.fromEntries(
    Object.entries(answers)
      ...
      .map(([k, v]) => [k.slice(prefix.length), v.noul ?? 0]),
  );
export const scores = (answers: Answers, prefix: string): Record<string, number> =>
  Object.fromEntries(
    Object.entries(answers)
      .filter(([k]) => k.startsWith(prefix))
      .map(([k, v]) => [k.slice(prefix.length), v.score ?? 0]),
  );
...
export const matchQuestion = (candidates: MatchCandidate[]): Choice => ({
  ...
  instructions: {
    candidates: Object.fromEntries(
      candidates.map((c) => [
        c.id,
        { title: c.title, era: c.era, place: c.place, description: c.description },
      ]),
    ),
    question:
      "Does `prompt` describe the same scenario as one of `candidates`, a close variant of one, or something new?",
  },
...
export const memberQuestion = (pack: Pack, m: Member, offer?: string): Noul => ({
  ...
  instructions: {
    ...
    ...(offer
      ? {
          offer_from_the_government: offer,
          question: `Given the offer, would this ${pack.vocabulary.member} vote yes on \`bill\` on the floor?`,
        }
  ...
  criteria: {
    ...
    false:
      "They vote no. The proposal hurts their core issues, patrons, or region, or their faction's leadership opposes it and nothing has been offered to them.",
  },
});
...
export function whipQuestions(pack: Pack, members: Member[]): Record<string, Question> {
  ...
  qs.filibuster = {
    type: "noul",
    instructions: `Would the opposition's leader block \`bill\` by procedure rather than lose the vote?`,
    criteria: {
      true: "The proposal is a major partisan priority the opposition strongly opposes.",
      false: "It is minor, cross-factional, or not worth the fight.",
    },
  };
  for (const b of pack.blocs)
    qs[`bloc_${b.id}`] = {
      ...
      instructions: {
        bloc: b.description,
        question: "How strongly does this group oppose `bill`?",
      },
      criteria: ["Indifferent or supportive", "Opposed", "Outraged"],
    };
  for (const p of pack.patrons)
    qs[`patron_${p.id}`] = {
      type: "score",
      instructions: {
        patron: p.name,
        wants: p.wants,
        hates: p.hates,
        question: "How strongly does this patron oppose `bill`?",
      },
      criteria: ["Indifferent or supportive", "Opposed", "Outraged"],
    };
  qs.constitutional = {
    type: "noul",
    instructions: `Does \`bill\` plainly exceed what the government of this polity may lawfully do?`,
    criteria: {
      true: "It clearly breaches a recognized right or limit on power in this polity.",
      false: "It is within ordinary legislative power, even if controversial.",
    },
  };
  const veto = pack.chamber.veto;
  if (veto)
    for (const m of members.filter((x) => x.flags.includes(veto.flag))) {
      qs[`veto_${m.id}`] = {
        type: "noul",
        instructions: {
          [pack.vocabulary.member]: persona(pack, m),
          power: veto.text,
          question: "Would they use this power against `bill`?",
        },
        criteria: {
          true: "The proposal threatens what they or their institution protect.",
          false: "They let it go to the vote.",
        },
      };
    }
  return qs;
...
export function whipState(pack: Pack, game: Game, bill: Bill) {
  ...
  return {
    ...
    government: {
      title: start?.seat_title ?? "the government",
      faction: name(game.faction),
      popularity: popularity(pack, game),
      // Measured -6 to -10 on co-factionals (v2 §4): the whip count must see a leadership that has turned.
      ...((game.holders[ownHolder(pack).id]?.support ?? 50) < 30
        ? { party_leadership: "hostile" }
        : {}),
    },
    [pack.vocabulary.chamber]: {
      largest_faction: name(largest),
      needed_to_pass: threshold(pack, game, bill),
...
const citizenPersona = (pack: Pack, c: Citizen) => ({
  ...
  job: c.job,
  town: c.town,
  region: pack.regions.find((r) => r.id === c.region)?.name ?? c.region,
  group: pack.blocs.find((b) => b.id === c.bloc)?.name ?? c.bloc,
...
export function citizenQuestions(
  pack: Pack,
  citizens: Citizen[],
  event: "vote" | "crisis",
  ...
  for (const c of citizens)
    qs[c.id] = {
      ...
      instructions: {
        ...
        question: `After ${what}, does this person approve of the government?`,
      },
      criteria: {
...
export const citizenState = (pack: Pack, game: Game, event: unknown) => ({
  event,
  record: record(pack, game),
});
...
export function holderQuestions(
  pack: Pack,
  game: Game,
  h: Holder,
  rows: { seats: Member[]; citizens: Citizen[] },
): Record<string, Question> {
  const title =
    pack.constitution?.ruler.role ??
    pack.starts.find((s) => s.faction === game.faction)?.seat_title ??
    "the government";
  const qs: Record<string, Question> = {};
  const criteria = {
  ...
  if (h.members === "seats") {
    for (const m of rows.seats)
      qs[`stance_${m.id}`] = {
        type: "noul",
        instructions: {
          [pack.vocabulary.member]: persona(pack, m),
          whip: Math.round(m.loyalty),
          question: `Would this ${pack.vocabulary.member} keep the ${title} in power?`,
        },
        criteria,
      };
    return qs;
...
export function eventQuestions(pack: Pack, scored: Storylet["scored"]): Record<string, Question> {
  const qs: Record<string, Question> = {};
  if (scored.includes("blocs"))
    for (const b of pack.blocs)
      qs[`bloc_${b.id}`] = {
        type: "score",
        instructions: {
          bloc: b.description,
          question: "How strongly does this group oppose the `stance` taken on `event`?",
        },
        criteria: ["Indifferent or supportive", "Opposed", "Outraged"],
      };
  if (scored.includes("patrons"))
    for (const p of pack.patrons)
      qs[`patron_${p.id}`] = {
        type: "score",
        instructions: {
          ...
          wants: p.wants,
          hates: p.hates,
          question: "How strongly does this patron oppose the `stance` taken on `event`?",
        },
        criteria: ["Indifferent or supportive", "Opposed", "Outraged"],
      };
  return qs;
  ...
  Object.fromEntries(
    Object.entries(answers)
      ...
      .map(([k, v]) => [
        ...
        Object.entries(v.probabilities!).sort((a, b) => b[1] - a[1])[0][0],
      ]),
  );
...
export function reactQuestions(pack: Pack, citizens: Citizen[]): Record<string, Question> {
  ...
  for (const c of citizens)
    qs[`react_${c.id}`] = {
      ...
      instructions: {
        citizen: citizenPersona(pack, c),
        question: "How does this person react to `post` from the government?",
      },
      options: Object.keys(REACTIONS),
    };
  return qs;
}
export const reactState = (pack: Pack, game: Game, text: string) => ({
  post: text,
  record: record(pack, game),
});
...
export function agreeQuestions(pack: Pack, citizens: Citizen[]): Record<string, Question> {
  ...
  for (const c of citizens)
    qs[`agree_${c.id}`] = {
      ...
      instructions: {
        citizen: citizenPersona(pack, c),
        question: "Which of the two posts in `duel` does this person agree with?",
      },
      options: ["government", "rival"],
    };
  return qs;
}
export const agreeState = (_pack: Pack, mine: string, rival: string) => ({
  duel: { government: mine, rival },
});

// The money in this person's own region sits in their question; the messages and the record sit in the state.
export function voteQuestions(
  pack: Pack,
  game: Game,
  citizens: Citizen[],
  spend: Record<string, number>,
  rivalSpend: Record<string, number>,
): Record<string, Question> {
  ...
  for (const c of citizens)
    qs[`vote_${c.id}`] = {
      ...
      instructions: {
        citizen: citizenPersona(pack, c),
        spend_here: spend[c.region] ?? 0,
        rival_spend_here: rivalSpend[c.region] ?? 0,
...
export const voteState = (pack: Pack, game: Game, messages: string[]) => ({
  campaign: { [pack.vocabulary.test]: pack.test.name, messages },
  record: record(pack, game),
});
