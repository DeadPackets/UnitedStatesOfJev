...
export function mkFrame(over: Partial<Frame> = {}): Frame {
  ...
  return {
    ...
    era: "1920s",
    place: "Harbor",
    description: "A council of the harbor city.",
    content_note: null,
    vocabulary: {
      seat: "seat",
      chamber: "council",
    ...
    factions,
    regions: weights.map((w, i) => ({
      id: `r${n(i)}`,
      name: `Region ${n(i)}`,
      weight: w,
      lean: factions.map((f) => ({ id: f.id, value: 0.1 })),
    })),
    ...
    starts: factions.map((f) => ({
      faction: f.id,
      seat_title: "Consul",
      coalition: [f.id],
      premise: "You hold the chair. The harbor is restless.",
      party: 50,
      capital: 50,
      hostile: [],
    })),
    test: {
      ...
      win: "You keep the chair.",
      lose: "You lose the chair.",
      reveal: "seats",
    },
    endings: {
      reelected: "Kept",
    ...
    lobby: {
      ...
      threat: {
        cost: 20,
        label: "Threaten a challenger",
        text: "Back a rival at the next canvass.",
      },
    },
    escalations: ESCALATION_KEYS.map((k) => ({
      key: k,
      name: k.replace(/_/g, " "),
      headline: `The ${k.replace(/_/g, " ")} sets in.`,
    })),
...
export const mkConstitution = (over: Record<string, unknown> = {}): Constitution =>
  ConstitutionSchema.parse({
    ...
    holders: [
      ...
      holder("league", {
        ...
        levers: ["spend", "favour"],
        line: 50,
        gives: { ledger: "treasury", amount: 4, per: "turn" },
        wants: ["tariffs"],
        redLines: ["piracy"],
      }),
    ],
    instruments: Object.fromEntries(
      VERBS.map((v) => [v, { name: v, consent: "none", price: { authority: 3 }, available: true }]),
    ),
    retention: {
      name: "the reckoning",
      weights: [
        { id: "council", value: 0.4 },
        { id: "street", value: 0.6 },
      ],
    },
    halfTerm: { holder: "council", name: "the halfway tide" },
    ledgers: {
      treasury: { name: "the harbour purse", line: 0 },
      authority: { name: "influence", line: 0 },
      chest: { name: "the war chest", line: 0 },
      loyalty: { name: "league mood", line: 20 },
      popularity: { name: "standing", line: 30 },
    },
    briefing: {
      situation: "The harbour is in dispute.",
      room: "Three bodies can stop you.",
      you: "You hold the chair.",
    },
    ...over,
  });
