...
export const CONTENT_RULE = `Content rule: no depiction, planning or reward of atrocities in bills, storylets, headlines or quotes. If the scenario seats the player in a regime defined by atrocities, keep the period and seat the player in the nearest governing seat that can be played at the strategic level, and say what you moved or left out in content_note. The factions that carried out a historical atrocity may exist as factions, named as historians name them; their members are written as ordinary politicians of the period doing ordinary political work, never as heroes and never glorified. Force is strategic only: a deployment, a curfew, martial law or the arrest of a named member. Never write the tactical detail of violence.
