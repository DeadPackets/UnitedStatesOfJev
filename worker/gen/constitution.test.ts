...
const ctx = () =>
  ({
    prompt: "a harbour city",
    lang: "en",
    fiction: false,
    facts: mkFacts(),
    frame: mkFrame(),
    calendar: { start_date: "0450-05-01", unit: "month" as const },
...
test("a ruler faction that is not a start id goes back for one repair round", async () => {
  ...
  try {
    await expect(constitution({ OPENROUTER_API_KEY: "t" } as never, ctx())).rejects.toThrow(
      /ruler\.faction/,
    );
