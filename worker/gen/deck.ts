...
const Effect = z.object({
  ...
  id: z.string().nullable(),
  delta: z.number().min(-15).max(15).nullable(),
  ...
  dated: z
    .array(
      z.object({
        ...
        title_hint: z.string(),
        stances: z.array(z.string()).min(1).max(3),
        scored: z.array(z.enum(["blocs", "patrons", "none"])).min(1),
        needs: z.array(Condition),
        results: z.array(Effect),
        memory: z.string().nullable(),
      }),
    )
    .min(5)
...
function resolve<T extends { id?: string | null }>(rows: readonly T[], ctx: GenCtx): T[] {
  ...
  ctx.frame.blocs.forEach((b, i) => {
    slots[`$bloc${i + 1}`] = b.id;
  });
  ctx.frame.patrons.forEach((p, i) => {
    slots[`$patron${i + 1}`] = p.id;
  });
  return rows.map((r) => (r.id && slots[r.id] ? { ...r, id: slots[r.id] } : { ...r }));
}

const pad = (s: string[], n: number) =>
  Array.from({ length: n }, (_, i) => s[Math.min(i, s.length - 1)]);
...
export async function deck(env: Env, ctx: GenCtx): Promise<Partial<GenCtx>> {
  ...
  const d = await luna(
    ...
    JSON.stringify({
      ...frameBrief(ctx),
      turn_unit: cal.unit,
      term: `${cal.start_date} plus 20 ${cal.unit}s`,
  ...
  const generic: Storylet[] = TEMPLATES.map((t, i) => {
    ...
    return {
      ...
      weight: 1,
      title_hint: g?.title_hint ?? t.note,
      stances: pad(g?.stances ?? ["Act", "Wait", "Refuse"], t.stances),
      scored: t.scored,
      needs: resolve(t.needs, ctx),
      results: resolve(t.results, ctx),
      memory: g?.memory ?? null,
    };
  });

  const dated: Storylet[] = d.dated
    .map((s, i) => ({
      id: `dat-${String(i + 1).padStart(2, "0")}`,
      kind: "dated" as const,
      date: s.date,
      turn: turnOf(s.date, cal.start_date, cal.unit),
      exogenous: s.exogenous,
      weight: 1,
      title_hint: s.title_hint,
      stances: s.stances,
      scored: s.scored,
      needs: resolve(s.needs, ctx),
      results: resolve(s.results, ctx),
      memory: s.memory,
    }))
