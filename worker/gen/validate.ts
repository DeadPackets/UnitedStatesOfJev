...
export function frame(f: Frame, sheet?: Facts | null, expectStart?: string | null): string[] {
  ...
  const fids = new Set(f.factions.map((x) => x.id)),
    tags = new Set(f.tags);
  const pids = new Set(f.patrons.map((p) => p.id)),
    rids = new Set(f.regions.map((r) => r.id));
  ...
  if (new Set(f.factions.map((x) => x.fill)).size !== f.factions.length)
    e.push("faction fills are not unique");
  ...
  if (seats !== f.chamber.size)
    e.push(`faction seats sum to ${seats}, not the chamber size ${f.chamber.size}`);
  if (f.chamber.threshold > f.chamber.size || f.chamber.supermajority <= f.chamber.threshold)
    e.push("threshold and supermajority are out of order");
  // A bar below a simple majority is a rule no chamber runs on: it passes a bill the room voted down.
  if (f.chamber.threshold < Math.ceil(f.chamber.size / 2))
    e.push(`threshold ${f.chamber.threshold} is below a majority of ${f.chamber.size}`);
  if (f.chamber.size > 30 && f.factions.length < 3)
    e.push(
      `only ${f.factions.length} factions for a chamber of ${f.chamber.size}; at least 3 are needed`,
    );

  for (const r of f.regions)
    for (const l of r.lean)
      if (!fids.has(l.id)) e.push(`region ${r.id} leans to unknown faction ${l.id}`);
  for (const p of f.patrons)
    for (const t of [...p.wants, ...p.hates])
      if (!tags.has(t)) e.push(`patron ${p.id} uses tag ${t}, which is not in tags`);
  ...
  if (new Set(f.promises.map((p) => p.tag)).size !== f.promises.length)
    e.push("two promises share a tag");
  ...
  for (const s of f.starts)
    for (const c of [...s.coalition, ...s.hostile])
      if (!fids.has(c)) e.push(`start ${s.faction} names unknown faction ${c}`);
  ...
  for (const x of f.factions) {
    ...
    if (p && p.alive_on_start_date === false)
      e.push(`leader "${x.leader}" (${x.id}) is not alive on the start date per the facts sheet`);
    else if (died !== null && start !== null && died < start)
      e.push(
        `leader "${x.leader}" (${x.id}) died ${p?.died}, before the start date ${f.start_date}`,
      );
  }
  ...
  if (!ymd(f.start_date)) e.push(`start_date ${f.start_date} is not a full date`);
    ...
    e.push(
      `no dated event from the facts sheet falls within a year after the start date ${f.start_date}`,
    );
  }
  return e;
}
...
const calendarHasAnchor = (start: string, facts: Facts) => {
  const s = ymd(start)!;
  return facts.dated_events.some((ev) => {
    const x = ymd(ev.date);
    return !!x && days(x) >= days(s) && days(x) - days(s) <= 366;
  });
};
...
export function constitution(c: Constitution, chamberExists: boolean): string[] {
  ...
  if (abroad < 1 || abroad > 4)
    e.push(`${abroad} holders abroad; 1 to 3 plus the international community are needed`);
  ...
  if (c.retention.weights.length < 2) {
    e.push(
      "fewer than two holders vote in the retention test: give at least two a weight between 0.15 and 0.6",
    );
  }
  if (c.retention.weights.reduce((a, w) => a + w.value, 0) <= 0) {
    e.push(
      "the retention weights total 0: give the holders that vote a weight between 0.15 and 0.6",
    );
  }
  for (const w of c.retention.weights) {
    if (w.value < 0.15 || w.value > 0.6)
      e.push(
        `holder ${w.id} has weight ${w.value}: every counted weight is between 0.15 and 0.6, and they sum to 1`,
      );
    const h = c.holders.find((x) => x.id === w.id);
    if (h && !h.levers.length)
      e.push(`counted holder ${w.id} has no lever: name at least one instrument that moves it`);
  }
  if (!c.holders.some((h) => h.id === c.halfTerm.holder))
    e.push(`halfTerm names ${c.halfTerm.holder}, which is not a holder`);
  if (
    !chamberExists &&
    Object.values(c.instruments).some(
      (i) => i.consent === "chamber" || i.consent === "chamber_supermajority",
    )
  ) {
