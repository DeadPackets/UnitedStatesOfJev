...
mock.module("../luna", () => ({
  luna: async (_env: unknown, _schema: unknown, _name: string, _system: string, user: string) => {
    ...
    seen.push(req);
    const take = (n: number, tag: string) =>
      Array.from({ length: n }, () => {
        series++;
        return (forceDup || (dupCitizens && tag === "Citizen")) && series % 2 === 0
          ? "Dup Name"
          : `${tag} ${series}`;
      });
    return {
      members: take(req.need.members, "Member").map((name) => ({
        name,
        gender: "woman",
        look: "Korean American",
      })),
      citizens: take(req.need.citizens, "Citizen"),
    };
  },
}));
...
describe("names", () => {
  test("60 members + 250 citizens: one member call, four 80-name citizen chunks, deduped, no shortfall", async () => {
    ...
    expect(seen.slice(1).map((s) => s.need)).toEqual([
      { members: 0, citizens: 80 },
      { members: 0, citizens: 80 },
      { members: 0, citizens: 80 },
      { members: 0, citizens: 10 },
    ]);
  ...
  test("each named member's face is drawn with its gender and look", async () => {
    ...
    const prompt = sheetPrompt(
      { era: "2020s", vocabulary: { member: "senators" } } as never,
      r.members!,
    );
  ...
  test("citizens short after the top-ups reuse the pool instead of failing the build", async () => {
    ...
    expect(r.citizens!.every((c) => c.name.startsWith("Citizen") || c.name === "Dup Name")).toBe(
      true,
    );
  });
});
