...
describe("clampChamberSize", () => {
  test("matches the prompt's worked examples", () => {
    expect(clampChamberSize(736)).toBe(92);
...
describe("frame chamber-size clamp", () => {
  test("a known real seat count overrides Luna's guess and rescales factions and thresholds", async () => {
    const facts = mkFacts({ bodies: [{ name: "The Council", size: 736, how_chosen: "elected" }] });
    const { frame: f } = await frame({} as never, ctx(facts));
    expect(f!.chamber.size).toBe(92);
    expect(f!.factions.reduce((s, x) => s + x.seats, 0)).toBe(92);
    expect(f!.chamber.threshold).toBeLessThan(f!.chamber.supermajority);
    expect(f!.chamber.supermajority).toBeLessThanOrEqual(92);
  });

  test("no numeric body leaves Luna's chamber size alone", async () => {
    const facts = mkFacts({ bodies: [] });
    const { frame: f } = await frame({} as never, ctx(facts));
    expect(f!.chamber.size).toBe(100);
  });
});
