...
export async function facts(env: Env, ctx: GenCtx): Promise<Partial<GenCtx>> {
  if (ctx.fiction)
    return { facts: { people: [], bodies: [], groupings: [], dated_events: [], anchor: -1 } };
  const sheet = await luna(
    env,
    FactsSchema,
    "facts",
...
export function mergeWikidata(
  sheet: Facts,
  people: { label: string; born: number | null; died: number | null }[],
): Facts {
  return {
    ...sheet,
    people: sheet.people.map((p) => {
      const w =
        people.find((q) => q.label === p.name) ??
        people.find((q) => last(q.label) === last(p.name));
      if (!w) return p;
      return {
        ...p,
        born: w.born !== null ? String(w.born) : p.born,
        died: w.died !== null ? String(w.died) : p.died,
      };
    }),
  };
}
