...
async function twins<T extends { id: string }>(
  env: Env,
  noun: string,
  batch: T[],
  strip: (r: T) => unknown,
  ...
  for (const [k, [a, b]] of flagged) {
    ...
    q2[k] = {
      type: "noul",
      instructions: {
        question: "Are `first` and `second` the same person described twice under different names?",
        first: strip(byId.get(a)!),
        second: strip(byId.get(b)!),
      },
      criteria: {
        true: "same life story, same manner, same interests, only the name differs",
        false: "different people who merely share a region, a faction or an issue",
      },
    };
  }
  const { answers: verdict } = await jev(env, state, q2);
  return [...flagged.entries()]
    .filter(([k]) => (verdict[k]?.noul ?? 0) >= 0.5)
    .map(([, pair]) => pair);
}
...
export async function dedupeMembers(env: Env, ctx: GenCtx): Promise<Member[]> {
  const found = (
    await Promise.all(chunk(ctx.members, BATCH).map((b) => twins(env, "members", b, stripMember)))
  ).flat();
  ...
  return rewrite(
    ...
    (m) => rank(m.seat),
    async (row, other) => {
      const [fresh] = await personas.members(env, ctx, [row], {
        must_differ_from: [`${other.name}: ${other.bio} ${other.tell}`],
      });
      return fresh ?? row;
    },
  );
...
export async function dedupeCitizens(env: Env, ctx: GenCtx): Promise<Citizen[]> {
  const found = (
    await Promise.all(
      chunk(ctx.citizens, BATCH).map((b) => twins(env, "citizens", b, stripCitizen)),
    )
  ).flat();
  ...
  return rewrite(
    ...
    (c) => rank(c.id),
    async (row, other) => {
      const [fresh] = await personas.citizens(env, ctx, [row], {
        must_differ_from: [`${other.name}: ${other.job} in ${other.town}. ${other.worldview}`],
      });
      return fresh ?? row;
    },
  );
...
export async function dedupe(env: Env, ctx: GenCtx): Promise<Partial<GenCtx>> {
  const [members, citizens] = await Promise.all([
    dedupeMembers(env, ctx),
    dedupeCitizens(env, ctx),
  ]);
