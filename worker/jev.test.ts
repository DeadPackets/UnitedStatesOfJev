...
const citizens = (): Citizen[] =>
  BLOCS.flatMap((bloc) =>
    Array.from({ length: 50 }, (_, i) => ({
      ...
      job: "docker",
      town: "Harbor City",
      worldview: "wants work",
      issues: ["tariffs", "fish-quotas"] as [string, string],
      weight: 1,
    })),
  );
...
const game = newGame(
  "g",
  encodeCode({ scenario: scenarioTag(mini.id), faction: 0, promises: [0, 1, 2], seed: 1 }),
  pack,
  "harborites",
  ["dockworker-pay", "tariffs", "fish-quotas"],
  { start_date: "0450-05-01", unit: "month" },
);
...
test("the duel asks the sample which post it agrees with", () => {
  ...
  expect((qs[`agree_${pack.citizens[0].id}`] as { options: string[] }).options).toEqual([
    "government",
    "rival",
  ]);
});
...
test("a citizen sees the money spent in their own region, not the whole map", () => {
  ...
  expect(
    JSON.stringify((qs[`vote_${here.id}`] as { instructions: unknown }).instructions),
  ).toContain("10");
  expect(
    JSON.stringify((qs[`vote_${there.id}`] as { instructions: unknown }).instructions),
  ).toContain('"spend_here":0');
});

test("choices reads the top probability and strips the prefix", () => {
  expect(
    choices(
      { react_a: { probabilities: { like: 0.2, boo: 0.7, share: 0.1 } }, other: { noul: 1 } },
      "react_",
    ),
  ).toEqual({ a: "boo" });
});
...
test("the meter counts one request's Jev tokens, its cost and its largest single call", async () => {
  ...
  globalThis.fetch = (async () =>
    Response.json({ answers: {}, usage: usage.shift() })) as unknown as typeof fetch;
  try {
    meter.reset();
    await jev({} as never, {}, {});
    await jev({} as never, {}, {});
