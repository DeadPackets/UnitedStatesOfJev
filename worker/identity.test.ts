...
test("a request with no cookie gets a fresh identity and the header that sets it", async () => {
  ...
  const back = await identity(
    SECRET,
    new Request("https://x/", { headers: { cookie: `other=1; ${IDENTITY_COOKIE}=${signed}` } }),
  );
...
test("a forged cookie is replaced, never trusted", async () => {
  const req = new Request("https://x/", {
    headers: { cookie: `${IDENTITY_COOKIE}=11111111-2222-3333-4444-555555555555.forged` },
  });
...
test("cookieOf reads only its own cookie", () => {
  expect(
    cookieOf(new Request("https://x/", { headers: { cookie: "a=1; usoj_id=abc; b=2" } })),
  ).toBe("abc");
