...
export function cells(sheetBytes: Uint8Array): Uint8Array[] {
  ...
  const w = Math.floor(img.get_width() / 4),
    h = Math.floor(img.get_height() / 4);
...
export const plate = (cell: Uint8Array, ink: Rgb, paper: Rgb) =>
  halftone(cell, 256, 256, ink, paper);
export const crest = (bytes: Uint8Array, color: Rgb, paper: Rgb) =>
  halftone(bytes, 256, 256, color, paper);
export const masthead = (bytes: Uint8Array, ink: Rgb, paper: Rgb) =>
  halftone(bytes, 1600, 400, ink, paper);
