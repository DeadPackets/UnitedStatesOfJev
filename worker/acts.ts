...
const v3Instrument = (pack: Pack, verb: Verb): Instrument | null =>
  verb === "law" || verb === "proclaim"
    ? {
        name: verb === "law" ? pack.vocabulary.bill : pack.vocabulary.post,
        vetoes: verb === "law" ? ["chamber"] : [],
        price: { authority: 0, treasury: 0, chest: 0 },
        available: true,
      }
    : null;
...
export function vetoesOf(pack: Pack, game: Game, verb: Verb): string[] {
  ...
  return game.emergency !== null && game.turn <= game.emergency
    ? v.filter((x) => !CHAMBER.has(x))
    : v;
}
  ...
  return vetoesOf(pack, game, verb).flatMap((veto): Veto[] => {
    if (CHAMBER.has(veto)) {
      ...
      const seats = Math.round(
        ((game.holders[chamber.id]?.support ?? 0) * pack.chamber.size) / 100,
      );
...
export const blocker = (
  ...
  verb: Verb,
  tokens?: Set<string>,
): Veto | undefined => vetoRows(pack, game, verb, tokens).find((veto) => !veto.agrees);
...
export function priceTag(pack: Pack, game: Game, q: Quote, member: string | null = null): PriceTag {
  ...
  const revenue = q.revenue.map((r) => ({
    ...r,
    delta: r.delta > 0 ? round1(r.delta * q.credibility) : r.delta,
  }));
    ...
    if (ch)
      wire.push(
        ...moveSupport(
          ...
          [ch.id],
          -SUPPORT_BYPASS,
          `${tag.title}, made without the chamber`,
        ),
      );
  }
...
export function favourCost(pack: Pack, _game: Game, m: Member): Price {
  ...
  return {
    authority: Math.round(base.authority * k),
    treasury: Math.round(base.treasury * k),
    chest: Math.round(base.chest * k),
  };
}
...
function applyForce(pack: Pack, game: Game, tag: PriceTag): WireLine[] {
  ...
  if (a) {
    ...
    wire.push(
      ...moveSupport(pack, game, [a.id], willing ? FORCE_ARMY_EASE : -FORCE_ARMY_RISE, tag.title),
    );
  }
...
function applyVerb(pack: Pack, game: Game, tag: PriceTag): WireLine[] {
  switch (tag.verb) {
    case "spend":
      return applySpend(pack, game, tag);
    case "favour":
      return applyFavour(pack, game, tag);
    case "force":
      return applyForce(pack, game, tag);
    default:
      return [];
  }
}
...
function applyTemplate(pack: Pack, game: Game, tag: PriceTag): WireLine[] {
  switch (tag.template) {
    ...
    case "state_media": {
      ...
      const pushed = holdersOf(pack)
        .filter((h) => h.response === "strike" || h.members === "patrons")
        .map((h) => h.id);
...
export function negotiate(
  ...
  const kept =
    game.deals?.term === game.term && game.deals.turn === game.turn ? game.deals.factions : [];
...
const lift = (tag: PriceTag, factions: string[]) => {
  for (const f of factions)
    tag.shift = { ...tag.shift, [f]: (tag.shift?.[f] ?? 0) + NEGOTIATE_LIFT };
...
export function keepDeals(game: Game, tag: PriceTag): void {
  ...
  if (tag.verb === "law" && deals?.term === game.term && deals.turn === game.turn)
    lift(tag, deals.factions);
}
  ...
  if (tag.verb === "law") {
    ...
    if (tag.revenue.length || tag.verb === "appoint") {
      enact(game, {
        ...
        title: tag.title,
        perTurn: tag.revenue,
        repealVetoes: vetoesOf(pack, game, tag.verb),
        sunset: tag.sunset,
      });
    }
