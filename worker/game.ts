...
export class GameDO extends DurableObject<Env> {
  ...
  async fetch(req: Request): Promise<Response> {
    ...
    try {
      ...
      if (body.turn !== undefined && body.turn !== game.turn)
        throw new Reject(409, "Stale turn. Reload the game.");
      ...
      try {
        switch (parts[0]) {
          case "bills":
            extra = await this.bill(game, pack, parts, body);
            break;
          case "acts":
            extra = await this.acts(game, pack, parts, body);
            break;
          case "events":
            if (parts[2] === "decline") {
              this.decline(game, pack, Number(parts[1]));
              break;
            }
            extra = await this.event(game, pack, Number(parts[1]), Number(body.stance));
            break;
          case "midterm":
            await this.midterm(game, pack);
            break;
          case "test":
            await this.term(s, pack);
            break;
          case "turn":
            if (parts[1] !== "end") throw new Reject(404, "Unknown action");
            await this.end(game, pack);
            break;
          case "continue":
            if (game.stage !== "won") throw new Reject(409, "The term is not won.");
            continueTerm(pack, game);
            s.prose = {};
          ...
          case "stop":
            ...
            break;
          default:
            throw new Reject(404, "Unknown action");
        }
      ...
      if (e instanceof UpstreamError)
        return Response.json({ error: "The chamber is in recess. Try again." }, { status: 503 });
  ...
  private async create(body: Record<string, unknown>): Promise<Saved> {
    ...
    const f =
      typeof body.faction === "number"
        ? body.faction
        : pack.factions.findIndex((x) => x.id === body.faction);
    ...
    if (promises.length !== 3 || new Set(promises).size !== 3)
      throw new Reject(400, `Pick three different ${pack.vocabulary.promise}s.`);
    const seed = Number.isInteger(body.seed)
      ? Number(body.seed) & 0x7fffffff
      : crypto.getRandomValues(new Uint32Array(1))[0] & 0x7fffffff;
    const code = encodeCode({
      scenario: scenarioTag(pack.id),
      faction: f,
      promises: promises as [number, number, number],
      seed,
    });
    ...
    const game = newGame(
      ...
      pack,
      start.faction,
      promises.map((p) => pack.promises[p].tag),
      pack.calendar,
      day ? { day } : undefined,
    );
    const platform = typeof body.platform === "string" ? body.platform.slice(0, 240) : "";
    if (platform.trim())
      for (const p of await platformPromises(this.env, pack, platform))
        authorPromise(game, p.tag, p.label, game.turn + p.window);
  ...
  private async load(): Promise<Saved> {
    ...
    const row = this.ctx.storage.sql
      .exec(
        "CREATE TABLE IF NOT EXISTS game(k TEXT PRIMARY KEY, v TEXT); SELECT v FROM game WHERE k='game'",
      )
      .toArray()[0];
  ...
  private save(s: Saved) {
    ...
    this.ctx.storage.sql.exec(
      "CREATE TABLE IF NOT EXISTS game(k TEXT PRIMARY KEY, v TEXT); INSERT OR REPLACE INTO game(k, v) VALUES ('game', ?)",
      JSON.stringify(s),
    );
  }
  ...
  private async loadPack(id: string): Promise<Pack> {
    ...
    if (!row.pack)
      throw new Reject(
        409,
        row.status === "ready" ? "That scenario is broken." : "That scenario is still building.",
      );
  ...
  private async bill(
    ...
    if (game.stage !== "session")
      throw new Reject(
        409,
        game.stage === "midterm"
          ? `The ${pack.vocabulary.midterm} comes first.`
          : "The term is over.",
      );
    ...
    if (!bill || bill.id !== game.turn || bill.votes)
      throw new Reject(409, `Not the current ${pack.vocabulary.bill}.`);
    ...
    if (owed && !spendCalls(game, owed))
      throw new Reject(
        409,
        `The clerks have done all they can this ${pack.vocabulary.turn}. End the turn.`,
      );
    try {
      switch (action) {
        case "whip":
        ...
        case "lobby":
          await this.lobby(
            ...
            body.action as LobbyAction,
          );
          return {};
        case "amend":
          parts[3] !== undefined
            ? this.adopt(bill, Number(parts[3]))
            : await this.amend(game, pack, bill);
          return {};
        case "vote":
          return this.vote(game, pack, bill);
        default:
          throw new Reject(404, "Unknown action");
      }
    } catch (e) {
    ...
    switch (parts[1] ?? "") {
      case "price":
        return this.price(
          ...
          pack,
          String(body.text ?? ""),
          body.verb as Verb | undefined,
          body.memberId as string | undefined,
        );
      case "":
        return this.doAct(game, pack);
      case "withdraw":
        return this.undoAct(game, pack, String(body.id ?? ""));
      case "negotiate":
        return this.deal(game, pack, String(body.faction ?? ""), String(body.term ?? ""));
      default:
        throw new Reject(404, "Unknown action");
    }
  }
  ...
  private async doAct(game: Game, pack: Pack): Promise<Extra> {
    ...
    if (tag.verb === "law" && game.phase !== "draft")
      throw new Reject(409, `A ${pack.vocabulary.bill} is already on the floor.`);
    if (!available(pack, game, tag.verb))
      throw new Reject(400, "That instrument is not available.");
    refuse(pack, game, tag.verb, actTokens(tag));
    if (!canAfford(pack, game, tag.charge))
      throw new Reject(402, "There is not enough to pay for that.");
    if (tag.verb === "law" && !tag.count && !spendCalls(game))
      throw new Reject(
        409,
        `The clerks have done all they can this ${pack.vocabulary.turn}. End the turn.`,
      );
    if (tag.verb === "proclaim" && game.posts.some((p) => p.turn === game.turn))
      throw new Reject(409, "One a turn.");
    if (tag.verb === "proclaim" && !spendCalls(game, 2))
      throw new Reject(
        409,
        `The clerks have done all they can this ${pack.vocabulary.turn}. End the turn.`,
      );
    ...
    if (tag.verb === "favour" && tag.member && game.phase === "whip") {
      ...
      if (bill?.whip && spendCalls(game)) {
        ...
        const r = await jev(this.env, whipState(pack, game, bill), {
          [m.id]: memberQuestion(pack, m),
        });
  ...
  private deal(game: Game, pack: Pack, faction: string, kind: string): Extra {
    ...
    if (!canAfford(pack, game, term.cost))
      throw new Reject(402, "There is not enough to pay for that.");
    if (!spendCalls(game))
      throw new Reject(
        409,
  ...
  private undoAct(game: Game, pack: Pack, id: string): Extra {
    ...
    if (!canAfford(pack, game, { authority: WITHDRAW_COST, treasury: 0, chest: 0 }))
      throw new Reject(402, "There is not enough to pay for that.");
  ...
  private async price(
    ...
    if (verb === "law" && game.phase !== "draft")
      throw new Reject(409, `A ${pack.vocabulary.bill} is already on the floor.`);
    if (verb === "proclaim" && game.posts.some((p) => p.turn === game.turn))
      throw new Reject(409, "One a turn.");
    ...
    if (!q.power || !q.era) {
      game.tag = null;
      game.refusal = {
        line: q.refusal ?? "That cannot be done here.",
        test: q.power ? "era" : "power",
        cost: REFUSAL_COST,
      };
      pushWire(
        game,
        pay(
          pack,
          game,
          { authority: REFUSAL_COST, treasury: 0, chest: 0 },
          "the clerk refused the act",
        ),
      );
      return {};
    }
  ...
  private async count(
    ...
    pack: Pack,
    bill: Bill,
    draft: BillDraft = bill,
  ): Promise<WhipCount> {
    const r = await jev(
      this.env,
      whipState(pack, game, { ...bill, ...draft }),
      whipQuestions(pack, game.members),
    );
    return {
      whip: Object.fromEntries(game.members.map((m) => [m.id, r.answers[m.id]?.noul ?? 0])),
      blocs: scores(r.answers, "bloc_"),
      patrons: scores(r.answers, "patron_"),
      vetoes: nouls(r.answers, "veto_"),
      filibuster: r.answers.filibuster?.noul ?? 0,
      constitutional: r.answers.constitutional?.noul ?? 0,
    };
  }
  ...
  private async lobby(game: Game, pack: Pack, bill: Bill, memberId: string, action: LobbyAction) {
    ...
    if (!m || !Object.hasOwn(LOBBY_COSTS, action))
      throw new Reject(400, `Bad ${pack.vocabulary.member} or action.`);
    ...
    if (game.ledgers.authority < lobbyCost(game, action))
      throw new Reject(402, `Not enough ${pack.vocabulary.capital}.`);
    const whip = bill.whip;
    const { offer } = applyLobby(pack, game, bill, m, action);
    const r = await jev(this.env, whipState(pack, game, bill), {
      [m.id]: memberQuestion(pack, m, offer),
    });
  ...
  private async amend(game: Game, pack: Pack, bill: Bill) {
    ...
    const whip = bill.whip;
    const opponents = game.members
      .filter((m) => (whip[m.id] ?? 0) < 0.5)
      .sort((a, b) => (whip[b.id] ?? 0) - (whip[a.id] ?? 0))
      .slice(0, 5);
    const loudest =
      Object.entries(bill.blocs ?? {}).sort((a, b) => b[1] - a[1])[0]?.[0] ?? pack.blocs[0].id;
    const drafts = await amendBill(
      this.env,
      pack,
      bill,
      opponents,
      pack.blocs.find((b) => b.id === loudest)?.name ?? loudest,
    ).catch((e) => {
      if (e instanceof UpstreamError) throw e;
    ...
    const amendments: Amendment[] = drafts.map((d, i) => ({
      ...
      count: counts[i],
      expected:
        Math.round(expectedYes(effectiveWhip(game, { ...bill, ...d, ...counts[i] })) * 10) / 10,
    }));
  ...
  private async vote(game: Game, pack: Pack, bill: Bill): Promise<Extra> {
    ...
    const defectors = game.members
      .filter((m) => m.faction === game.faction && !votes[m.id])
      .sort((a, b) => (bill.whip![b.id] ?? 0) - (bill.whip![a.id] ?? 0))
      .slice(0, 3);
    const verdict = {
    ...
    const [headline, said, citizens] = await Promise.all([
      ...
      jev(
        this.env,
        citizenState(pack, game, verdict),
        citizenQuestions(pack, pack.citizens, "vote"),
      ),
    ]);
  ...
  private async midterm(game: Game, pack: Pack) {
    if (game.stage !== "midterm")
      throw new Reject(409, `The ${pack.vocabulary.midterm} is not due.`);
    const r = await jev(
      this.env,
      voteState(pack, game, []),
      voteQuestions(pack, game, pack.citizens, {}, {}),
    );
    ...
    const head = await halfTerm(this.env, pack, {
      ...
      seats_changed: draw.lost.length,
      seats_up: draw.up.length,
  ...
  private async proclaim(game: Game, pack: Pack, tag: PriceTag) {
    ...
    const r = await jev(
      this.env,
      reactState(pack, game, text),
      reactQuestions(pack, pack.citizens),
    );
    const raw = choices(r.answers, "react_");
    const reactions = Object.fromEntries(
      Object.entries(raw).map(([id, o]) => [id, REACTIONS[o] ?? "ignore"]),
    ...
    const said = await replies(this.env, pack, text, loudest, record(pack, game)).catch(() => ({
      replies: [],
      rival: "",
    }));
    const duel = said.rival
      ? choices(
          (await jev(this.env, agreeState(pack, text, said.rival), agreeQuestions(pack, sample)))
            .answers,
          "agree_",
        )
      : {};
    const post = applyPost(
      ...
      reactions,
      said,
      duel as Record<string, "government" | "rival">,
      tag,
    );
  ...
  private async event(game: Game, pack: Pack, i: number, stance: number): Promise<Extra> {
    ...
    if (!(Number.isInteger(stance) && stance >= 0 && stance < event.stances.length))
      throw new Reject(400, "Pick a stance.");
    if (!spendCalls(game, 2))
      throw new Reject(
        409,
        `The clerks have done all they can this ${pack.vocabulary.turn}. End the turn.`,
      );
    ...
    const state = {
      event: event.card ?? { title: storylet?.title_hint ?? event.id },
      stance: taken,
    ...
    const [line, citizens] = await Promise.all([
      ...
      jev(
        this.env,
        citizenState(pack, game, { ...state.event, stance_taken: taken }),
        citizenQuestions(pack, pack.citizens, "crisis"),
      ),
    ]);
  ...
  private async readHolders(game: Game, pack: Pack) {
    ...
    const rows = holdersOf(pack)
      .filter((h) => moved.has(h.id))
      .slice(0, callsLeft(game));
    ...
    const reads = await Promise.all(
      rows.map(async (h) => {
        const sample = {
          ...
          citizens:
            h.members === "citizens" ? streetSample(game, pack.citizens, HOLDER_SAMPLE) : [],
        };
        const r = await jev(
          this.env,
          holderState(pack, game, h),
          holderQuestions(pack, game, h, sample),
        );
      ...
      if (h)
        pushWire(
          ...
          moveSupport(
            ...
            game,
            [id],
            clamp(clamp(s, 0, 1) * 100 - h.support, -REREAD_MAX, REREAD_MAX),
            "read again at the turn's end",
          ),
        );
    }
  }
  ...
  private async end(game: Game, pack: Pack) {
    if (game.stage !== "session") throw new Reject(409, "Not now.");
    if (game.events.some((e) => e.stance === undefined))
      throw new Reject(409, "Answer the card on the desk first.");
    ...
    if (out.event?.kind === "foreign" && out.event.holder) {
      ...
      if (h)
        out.event.card = await cardText(
          this.env,
          pack,
          foreignStorylet(pack, game, h),
          record(pack, game),
        ).catch(() => undefined);
    } else if (out.event) {
      const storylet = deckOf(pack, game).find((s) => s.id === out.event!.id);
      if (storylet)
        out.event.card = await cardText(this.env, pack, storylet, record(pack, game)).catch(
          () => undefined,
        );
    }
  }
  ...
  private async term(s: Saved, pack: Pack) {
    ...
    endTerm(
      pack,
      game,
      game.earlyTest ? earlyTest(pack, game, game.earlyTest) : runTest(pack, game),
    );
  }
  ...
  private async epilogue(s: Saved, pack: Pack) {
    ...
    if (game.mode === "daily" && game.term === 1)
      await endPlay(
        this.env,
        game.id,
        JSON.stringify(runStyle(pack, game).grid),
        game.test?.won ?? false,
      );
    if (s.prose.ending) return;
    const state = {
      ...record(pack, game),
      mandate: game.test ? Math.round(game.test.mandate * 100) : null,
...
export function migrate(game: Game, pack: Pack): void {
  ...
  if (L && (L.loyalty !== undefined || L.popularity !== undefined)) {
    game.ledgers = {
      treasury: Number(L.treasury ?? 0),
      authority: Number(L.authority ?? 0),
      chest: Number(L.chest ?? 0),
    };
    game.regions = (L.popularity as Record<string, number>) ?? {};
    const seeded = seedHolders(pack, game, Number(L.loyalty ?? 50));
    const old = (game.holders ?? {}) as Record<
      string,
      { stance?: number; warnedAt?: number | null }
    >;
    const kept = (h: Holder) =>
      h.members !== "seats" && h.id !== publicHolder(pack)?.id && old[h.id]?.stance !== undefined;
    game.holders = Object.fromEntries(
      holdersOf(pack).map((h) => [
        h.id,
        {
          ...seeded[h.id],
          ...(kept(h) ? { support: Math.round(old[h.id].stance! * 100) } : {}),
        },
      ]),
    );
  ...
  for (const law of game.inForce as ((typeof game.inForce)[number] & {
    repealConsent?: string;
  })[]) {
...
const room = (pack: Pack, game: Game): HolderView[] => {
  ...
  return holdersOf(pack).map((h) => {
    ...
    return {
      ...
      name: h.name,
      where: h.where,
      support: s?.support ?? 50,
      line: s?.line ?? h.line,
      response: s?.response ?? h.response,
      weight: s?.weight ?? weightOf(pack, h.id),
      levers: h.levers,
      warnedAt: s?.warnedAt ?? null,
      nearest: h.id === near,
  ...
  for (const v of VERBS) {
    ...
    if (i)
      out[v] = {
        ...i,
        affordable:
          available(pack, game, v) && !blocker(pack, game, v) && canAfford(pack, game, i.price),
      };
  }
  return out;
};
...
export function view(pack: Pack, { game, prose }: Saved, extra: Extra = {}) {
  ...
  return {
    ...
    ) as Record<LobbyAction, number>,
    holders: room(pack, game),
    instruments: instrumentRows(pack, game),
    bar: testBar(pack, game),
    ruler: pack.constitution?.ruler ?? {
      role: start?.seat_title ?? "the government",
    ...
    bills: bills.map((b) => {
      ...
      if (cur.id < game.turn - 1)
        return { ...cur, whip: undefined, votes: undefined, quotes: undefined };
      ...
      return {
        ...cur,
        whip,
        expected: Math.round(expectedYes(whip) * 10) / 10,
        needed: threshold(pack, game, cur),
        band: whipBand(whip),
      };
    }),
    citizens: pack.citizens.map(({ id, region, bloc, name, weight }) => ({
      id,
      region,
      bloc,
      name,
