...
async function artStep(env: Env, id: string, ctx: GenCtx): Promise<Pack["art"]> {
  ...
  const [head, ...crests] = await Promise.all([
    muse(
      env,
      `wide ${ctx.frame.era} engraving of a landmark of ${ctx.frame.place}, ${NO_TEXT}`,
      "4:1",
    ).then((b) => put(env, `scenarios/${id}/masthead.png`, mastheadPlate(b, ink, paper))),
    ...ctx.frame.factions.map((f) =>
      muse(env, `${f.name} emblem, flat, centered, ${ornament}, ${NO_TEXT}`, "1:1").then((b) =>
        put(env, `scenarios/${id}/crests/${f.id}.png`, crest(b, rgb(f.color), paper)),
      ),
    ),
  ]);
  return {
    masthead: head,
    crests: ctx.frame.factions.map((f, i) => ({ id: f.id, value: crests[i] })),
    portraits: [],
  };
}
...
export async function portraitSheet(
  env: Env,
  scenario: string,
  pack: Pack,
  group: Member[],
): Promise<boolean> {
  const ink = rgb(pack.theme.ink),
    paper = rgb(pack.theme.paper);
  try {
...
async function portraitsStep(env: Env, id: string, pack: Pack) {
  ...
  await Promise.all(
    chunk(pack.members, SHEET).map(async (group, gi) => {
...
export class ScenarioBuild extends WorkflowEntrypoint<Env, BuildParams> {
  async run(event: WorkflowEvent<BuildParams>, step: WorkflowStep) {
    ...
    try {
      ...
      const art = await stage(
        "art",
        (e) => artStep(e, id, ctx),
        (r) => ({ kind: "art", masthead: r.masthead, crests: r.crests.length }),
      );
