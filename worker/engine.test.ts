...
const setPublic = (g: Game, v: number) => {
  for (const r of REGIONS) movePopularity(pack, g, [r], v - g.regions[r], "set");
};
...
const busy = (g: Game) =>
  g.acts.push({
    ...
    reading: "",
    credibility: 1,
    charge: { authority: 0, treasury: 0, chest: 0 },
  });
const setSupport = (g: Game, s: Record<string, number>) => {
  for (const [id, v] of Object.entries(s))
    if (id === "street") setPublic(g, v);
    else g.holders[id].support = v;
};
const bill = (g: Game, p: number, extra: Partial<Bill> = {}): Bill => {
  const b: Bill = {
    ...
    tags: ["tariffs"],
    offers: {},
    whip: Object.fromEntries(g.members.map((m) => [m.id, p])),
    filibuster: 0,
    constitutional: 0,
    ...extra,
  };
...
test("the code round-trips", () => {
  ...
  expect(
    decodeCode(encodeCode({ scenario: "ZZZZZZ", faction: 11, promises: [7, 0, 3], seed: 1 }))
      .faction,
  ).toBe(11);
...
test("a new game opens three resources and one number per group (R24)", () => {
  ...
  const v2 = PackSchema.parse({
    ...pack,
    constitution: {
      ...pack.constitution!,
      ownGroup: "guard",
      holders: pack.constitution!.holders.map((h) =>
        h.id === "guard" ? { ...h, support: 62, line: 45 } : h,
      ),
    },
  });
...
test("a hostile partner votes as opposition and grudges stick", () => {
  ...
  expect(effectiveWhip(g, b)[g.members.find((m) => m.faction === "harborites")!.id]).toBeCloseTo(
    0.85,
    5,
  );
});
...
test("a passed bill moves the five ledgers", () => {
  ...
  const b = bill(g, 0, {
    whip: Object.fromEntries(g.members.map((m) => [m.id, back(m)])),
    blocs: { dockworkers: 0, merchants: 2 },
    patrons: { "grand-exchange": 0, "keelwright-hall": 2 },
  });
  applyVote(pack, g, b);
  endTurn(pack, g);
  ...
  applyVote(
    pack,
    g,
    bill(g, 0, { whip: Object.fromEntries(g.members.map((m) => [m.id, back(m)])) }),
  );
...
test("a missed promise decays popularity by a share a turn, never a cliff", () => {
  ...
  for (const r of REGIONS.filter((x) => x !== g.rival!.region))
    expect(g.regions[r]).toBeCloseTo(44.3, 5);
});
...
test("an authored promise carries its own window and share", () => {
  ...
  expect(g.promises["harbor-tolls"]).toEqual({
    label: "Cut the tolls by the spring",
    passed: 0,
    state: "pending",
    window: 6,
    share: 0.05,
...
test("the Director keeps 4 to 7 crises a term and never two in a row before turn 17", () => {
  ...
  for (let t = 0; t < 200; t++) {
    ...
    for (let i = 1; i < crises.length; i++) {
      ...
      if (crises[i].turn < 17 && !exo.has(crises[i].id))
        expect(crises[i].turn - crises[i - 1].turn).toBeGreaterThan(1);
    }
...
test("a minority start prints its shortfall, is handicapped over 6 and wins on survival over 15", () => {
  const thin = (threshold: number, own: number): Pack => ({
    ...
    members: pack.members.map((m, i) => ({
      ...m,
      faction: i < own ? "harborites" : "keelwrights",
    })),
  });
...
test("an early test brings its caller in and renormalises the weights", () => {
  ...
  expect(r.holders.find((h) => h.id === "guard")!.weight).toBeCloseTo(
    EARLY_WEIGHT / (1 + EARLY_WEIGHT),
    5,
  );
  ...
  expect([g.stage, g.earlyTest, g.result, g.terms.length]).toEqual([
    "midterm",
    undefined,
    undefined,
    0,
...
test("another term carries the laws and every group's support, and reseeds the half-term class", () => {
  ...
  enact(g, {
    id: "l1",
    verb: "law",
    title: "The harbour levy",
    perTurn: [{ ledger: "treasury", delta: 6 }],
    repealVetoes: ["chamber"],
    sunset: null,
  });
...
test("every escalation of the twenty has a hook or a stored number", () => {
  ...
  expect(
    g.members.filter((m) => m.situation === "is under investigation for corruption").length,
  ).toBeGreaterThanOrEqual(4);
  expect(Object.keys(g.stageB).sort()).toEqual([
    "apathy",
    "loud_opposition",
    "rival_surge",
    "split_chamber",
...
test("the seeded region lists differ by seed", () => {
  ...
  for (let seed = 1; seed <= 10; seed++) {
    const g = newGame(
      "g",
      encodeCode({ scenario: scenarioTag(mini.id), faction: 0, promises: [0, 1, 2], seed }),
      pack,
      "harborites",
      PROMISES,
      CAL,
...
const datedPack = (turn: number, exogenous: boolean): Pack => ({
  ...
  deck: [
    ...pack.deck
      ...
      .map((s) => ({
        ...s,
        needs: [{ ledger: "turn" as const, id: null, op: "<" as const, value: 0 }],
      })),
    {
      id: "dat-x",
      kind: "dated",
...
test("a hostile party shows in the whip state, and a favor comes back as capital", () => {
  ...
  applyVote(
    pack,
    h,
    bill(h, 0, { whip: Object.fromEntries(h.members.map((x) => [x.id, x.id === m.id ? 1 : 0])) }),
  );
...
test("a storylet streak effect counts toward the best streak", () => {
  const p: Pack = {
    ...pack,
    deck: pack.deck.map((s) =>
      s.id === "gen-01"
        ? {
            ...s,
            results: [{ ledger: "streak" as const, id: null, delta: 3, set: null, chance: null }],
          }
        : s,
    ),
  };
...
const agreeAll = (who: "government" | "rival") =>
  Object.fromEntries(pack.citizens.slice(0, 50).map((c) => [c.id, who]));
...
const tagFor = (over: Partial<PriceTag> = {}): PriceTag => ({
  verb: "proclaim",
  title: "A notice",
  reading: "You put up a notice.",
  credibility: 1,
  quoted: { authority: 0, treasury: 0, chest: 0 },
  charge: { authority: 0, treasury: 0, chest: 2 },
  discounted: false,
  revenue: [],
  serves: [],
  hits: [],
...
const reactMix = (like: number, boo: number) => {
  ...
  pack.citizens.forEach((c, i) => {
    out[c.id] = pct(i) < like ? "like" : pct(i) < like + boo ? "boo" : "ignore";
  });
  return out;
};
...
test("losing the post duel is recorded on the post", () => {
  const g = game();
  expect(applyPost(pack, g, 1, "x", react("ignore"), said, agreeAll("rival"), tagFor()).won).toBe(
    false,
  );
  expect(
    applyPost(pack, g, 2, "x", react("ignore"), said, agreeAll("government"), tagFor()).won,
  ).toBe(true);
});
...
test("a forced seat never flips to a coalition partner still inside ownSide", () => {
  ...
  const friendly: Pack = {
    ...pack,
    starts: pack.starts.map((s) => (s.faction === "tidebound" ? { ...s, hostile: [] } : s)),
  };
  const g = game();
  g.stageB.split_chamber = 8;
...
test("the midterm swaps only the seats it lost, and the new members start empty", () => {
  ...
  const personas: Persona[] = slots.map((s) => ({
    id: s.id,
    name: `New ${s.id}`,
    bio: "b",
    tell: "t",
    core_issues: [pack.tags[0]],
  }));
...
test("opposition seats swapping among themselves never end a run", () => {
  ...
  const crossed = draw.lost.reduce(
    (a, l) => a + Number(l.to === "harborites") - Number(l.from === "harborites"),
    0,
  );
  applyMidterm(
    pack,
    g,
    draw,
    replacements(pack, g, draw).map((s) => ({
      id: s.id,
      name: "n",
      bio: "b",
      tell: "t",
...
test("losing 40% of the class on the government's own side ends the run as a lame duck with a scored term", () => {
  ...
  applyMidterm(
    ...
    g,
    draw,
    replacements(friendly, g, draw).map((s) => ({
      id: s.id,
      name: "n",
      bio: "b",
      tell: "t",
      core_issues: [friendly.tags[0]],
    })),
  );
...
test("each resource has a failure line, and support is no resource", () => {
  ...
  g.ledgers.chest = 0; // both lines are 0, so empty is already at the line
  expect(belowLine(pack, g)).toEqual(["treasury", "chest"]);
  g.ledgers.treasury = 5;
  g.ledgers.chest = 5;
  g.holders.own.support = 0;
  setPublic(g, 0);
...
test("spec section 4's sources pay: a kept promise, and the chest capped per verdict", () => {
  ...
  applyVote(
    pack,
    h,
    bill(h, 0, { whip: Object.fromEntries(h.members.map((m) => [m.id, back(m)])) }),
  );
...
test("support moves one way for every group: the chamber in whole seats, the public by region", () => {
  ...
  expect(moveSupport(pack, g, ["own"], 3, "a kept word")[0]).toMatchObject({
    id: "own",
    ledger: "loyalty",
    delta: 3,
  });
...
test("a card is declined for 3 support with the group its answers would please most (R33)", () => {
  ...
  expect([foreign.stance, foreign.declined, g.holders.league.support]).toEqual([
    -1,
    true,
    45 - DECLINE_COST,
  ]);
  ...
  const card = (results: Pack["deck"][number]["results"]): Pack => ({
    ...pack,
    deck: pack.deck.map((s) => (s.id === "gen-01" ? { ...s, results } : s)),
  });
  const own = g.holders.own.support,
    street = { ...g.regions };
  declineEvent(
    card([
      { ledger: "party", delta: 5 },
      { ledger: "approval", delta: 2 },
    ]),
    g,
    { id: "gen-01", turn: 1, relief: false, kind: "crisis", stances: ["a", "b"] },
  );
  ...
  declineEvent(card([{ ledger: "chest", delta: 5 }]), g, {
    id: "gen-01",
    turn: 1,
    relief: false,
    kind: "swan",
...
test("a rate an empty treasury cannot pay prints the move that landed, so the grid does not count it", () => {
  ...
  enact(g, {
    id: "l1",
    verb: "law",
    title: "Grain for the quay",
    perTurn: [{ ledger: "treasury", delta: -15 }],
    repealVetoes: ["chamber"],
    sunset: null,
  });
  endTurn(pack, g);
...
test("an authored sunset lapses the law on its own", () => {
  ...
  enact(g, {
    id: "l2",
    verb: "decree",
    title: "A two tide curfew",
    perTurn: [{ ledger: "popularity", delta: -2 }],
    repealVetoes: [],
    sunset: 2,
  });
  endTurn(pack, g);
...
test("a popularity rate moves the public's regions and a loyalty rate your own side", () => {
  const g = game();
  const before = { ...g.regions },
    own = g.holders.own.support;
  enact(g, {
    id: "l3",
    verb: "law",
    title: "Relief for the quay",
    perTurn: [
      { ledger: "popularity", delta: 1 },
      { ledger: "loyalty", delta: 2 },
    ],
    repealVetoes: ["chamber"],
    sunset: null,
  });
  ...
  for (const r of REGIONS)
    expect(g.regions[r]).toBe(before[r] + 1 - (r === g.rival!.region ? RIVAL_HIT : 0));
...
test("a court that strikes takes the newest law in force with it", () => {
  ...
  enact(g, {
    ...
    verb: "law",
    title: "The old levy",
    perTurn: [{ ledger: "treasury", delta: 1 }],
    repealVetoes: ["chamber"],
    sunset: null,
  });
  enact(g, {
    id: "l5",
    verb: "decree",
    title: "The new curfew",
    perTurn: [{ ledger: "popularity", delta: -1 }],
    repealVetoes: [],
    sunset: null,
  });
...
test("the record is bounded, and drops its softest lines first", () => {
  ...
  for (let i = 0; i < 40; i++) {
    g.bills.push({
      ...
      tags: [],
      offers: {},
      headline: { title: `A long headline about decree ${i} and the harbour`.repeat(4), lede: "" },
    });
    enact(g, {
      id: `l${i}`,
      verb: "law",
      title: `A law with a long name number ${i}`.repeat(3),
      perTurn: [{ ledger: "treasury", delta: 1 }],
      repealVetoes: [],
      sunset: null,
    });
...
test("the boundary clears the price tag, the refusal and the turn's swing", () => {
  ...
  g.tag = {
    verb: "decree",
    title: "A levy",
    reading: "Raise the levy.",
    credibility: 1,
    quoted: { authority: 0, treasury: 0, chest: 0 },
    charge: { authority: 3, treasury: 0, chest: 0 },
    discounted: false,
    revenue: [],
    serves: [],
    hits: [],
...
test("a black swan waits for a clear turn and fires at most once a term", () => {
  ...
  const swans = [1, 2, 3].map((i) => ({
    ...
    kind: "swan" as const,
    weight: 1,
    title_hint: "The mole gives way in a night storm",
    stances: ["Rebuild it now", "Let the ships wait"],
...
test("the turn's row is the resource or public or own support that moved most; other groups do not count", () => {
  ...
  ).toEqual({ turn: 4, ledger: "popularity", delta: 9, cause: "relief" });
  expect(
    biggestMove([{ kind: "support", id: "army", delta: 30, cause: "the curfew" }], 2).ledger,
  ).toBe("quiet");
...
test("a stored pack whose holders say stance 0 still opens every holder inside the band", () => {
  const zeroed = PackSchema.parse({
    ...pack,
    constitution: {
      ...pack.constitution!,
      holders: pack.constitution!.holders.map((h) => ({ ...h, stance: 0 })),
    },
  });
