...
export interface Bill extends BillDraft {
  ...
  acts?: Record<string, LobbyAction>;
  ...
  struck?: boolean;
  vetoed?: boolean;
  headline?: { title: string; lede: string };
  quotes?: { name: string; text: string }[];
  rates?: InForce["perTurn"];
  keeps?: string[];
...
export type WhipCount = Pick<
  Bill,
  "whip" | "blocs" | "patrons" | "filibuster" | "constitutional" | "vetoes"
>;
...
export interface Game {
  ...
  term: number;
  turn: number;
  stage: "session" | "midterm" | "test" | "won" | "over";
  ...
  escalations: EscalationKey[];
  stageB: Partial<Record<EscalationKey, number>>;
...
export interface WireLine {
  kind: "ledger" | "support" | "promise" | "card";
  ledger?: LedgerV4;
  id?: string | null;
  region?: string;
  delta: number;
...
export const ledgerLine = (pack: Pack, l: Resource): number =>
  pack.constitution?.ledgers[l].line ?? LEDGER_LINES[l];

export function belowLine(pack: Pack, game: Game): Resource[] {
  return RESOURCES.filter((l) => {
    const v = game.ledgers[l],
      line = ledgerLine(pack, l);
...
export const canAfford = (_pack: Pack, game: Game, price: Price): boolean =>
  game.ledgers.authority >= price.authority &&
  game.ledgers.treasury >= price.treasury &&
  game.ledgers.chest >= price.chest;
...
export const SITUATIONS = [
  "faces a challenger for their seat this term",
  "is retiring at the end of this term",
  "just lost their largest patron",
  "their region was hit by disaster this month",
  "is under investigation for corruption",
  "wants a higher office",
  "their region's trade has collapsed",
  "was just given a seat on the treasury board",
  "leads their faction's line on this issue",
  "is feuding in public with the ruler",
  "owes the ruler a favor from last term",
  "has a brother in the opposition",
];
...
const RELIEF = new Set(
  TEMPLATES.flatMap((t, i) =>
    t.id.startsWith("relief-") ? [`gen-${String(i + 1).padStart(2, "0")}`] : [],
  ),
);
...
export function decodeCode(code: string): Code {
  const m =
    typeof code === "string" &&
    /^J3-([0-9A-Z]{6})-([0-9A-Z])-([0-9A-Z]{3})-([0-9A-Z]{6})$/.exec(code.trim().toUpperCase());
...
export function newGame(
  ...
  const game: Game = {
    ...
    regions: Object.fromEntries(
      pack.regions.map((g) => [
        g.id,
        clamp(Math.round(50 + leanOf(pack, g.id, start.faction) * 15 + (r() - 0.5) * 6), 20, 80),
      ]),
    ),
    ...
    promises: Object.fromEntries(
      promises.map((t) => [
        t,
        {
          label: pack.promises.find((p) => p.tag === t)?.label ?? t,
          passed: 0,
    ...
    members: pack.members.map((m) => ({
      ...m,
      memory: [],
      loyalty: loyaltyFor(start, m.faction, start.faction),
      mood: 0,
    })),
    bills: [],
    posts: [],
    events: [],
    director: { intensity: 0, lastCrisis: -1, seen: [], swan: null },
    streak: 0,
    bestStreak: 0,
  ...
  for (const m of shuffled.slice(0, Math.max(1, Math.round(pack.chamber.size * 0.15))))
    m.situation = SITUATIONS[Math.floor(r() * SITUATIONS.length)];
...
const v3Room = (pack: Pack): Holder[] =>
  (["seats", "citizens"] as const).map((members) => {
    const name = members === "seats" ? pack.vocabulary.chamber : "the street";
    return {
      id: members === "seats" ? "chamber" : "street",
      name,
      where: "home",
      persona: { name, role: name, bio: "", tell: "" },
      members,
      stance: 0.5,
      line: 100,
      response: "none",
...
const ownOf = (pack: Pack): Holder => {
  ...
  return {
    ...
    stance: 0.5,
    support: 50,
    line: pack.constitution?.ledgers.loyalty?.line ?? 20,
    response: "none",
    levers: [],
    wants: [],
    redLines: [],
...
export const publicHolder = (pack: Pack): Holder | null => {
  ...
  return (
    hs.find((h) => h.id === pack.constitution?.publicGroup) ??
    hs.find((h) => h.members === "citizens") ??
    null
  );
};
...
export const chamberHolder = (pack: Pack): Holder | null =>
  holdersOf(pack).find((h) => h.members === "seats") ?? null;

// Every holder opens near even: the live Biden pack stored every stance at 0, which no ruler can pass the test from.
export const STANCE_LO = 0.3,
  STANCE_HI = 0.7; // TUNE
...
const chamberShare = (pack: Pack, game: Game) =>
  pack.chamber.size
    ? round1(
        (game.members.filter((m) => ownSide(pack, game, m.faction)).length / pack.chamber.size) *
          100,
      )
    : 50;
...
export function seedHolders(pack: Pack, game: Game, party: number): Record<string, HolderState> {
  ...
  return Object.fromEntries(
    holdersOf(pack).map((h) => {
      const support =
        h.members === "seats"
          ? chamberShare(pack, game)
          : h.id === pub
            ? round1(nationalPopularity(pack, game))
            : h.id === OWN
              ? party
      ...
      const line =
        h.support !== undefined
          ? h.line
          : Math.max(0, Math.min(100 - h.line, Math.round(support) - LEGACY_ROOM));
      return [
        h.id,
        {
          id: h.id,
...
const hueOf = (pack: Pack, id: string): { ledger?: LedgerV4 } =>
  id === publicHolder(pack)?.id
    ? { ledger: "popularity" }
    : id === ownHolder(pack).id
      ? { ledger: "loyalty" }
      : {};
...
export function movePopularity(
  ...
  return rs.map((r) => ({
    ...
    id,
    ledger: "popularity" as const,
    region: r.id,
    delta: bump(pack, game, r.id, delta),
    cause,
  }));
}
...
export function rivalMove(pack: Pack, game: Game): { move: RivalMove; wire: WireLine[] } | null {
  ...
  for (const m of game.members)
    if (!mine.has(m.faction)) seats.set(m.faction, (seats.get(m.faction) ?? 0) + 1);
  ...
  if (!seats.size)
    for (const m of game.members)
      if (m.faction !== game.faction) seats.set(m.faction, (seats.get(m.faction) ?? 0) + 1);
  const id =
    [...seats.entries()].sort((a, b) => b[1] - a[1])[0]?.[0] ??
    pack.factions.find((f) => f.id !== game.faction)?.id;
  const f = id ? pack.factions.find((x) => x.id === id) : undefined;
  if (!f) return null;
  const home = holdersOf(pack).filter((h) => h.where === "home");
  const room = (id: string) =>
    game.holders[id] ? game.holders[id].support - game.holders[id].line : 100;
  const backer = [...home].sort((a, b) => room(a.id) - room(b.id))[0];
  const weakest = [...pack.regions].sort(
    (a, b) => (game.regions[a.id] ?? 50) - (game.regions[b.id] ?? 50),
  )[0];
...
export const armyHolder = (pack: Pack): Holder | null =>
  holdersOf(pack).find((h) => h.levers.includes("force")) ?? null;
...
export function advanceWarnings(
  pack: Pack,
  game: Game,
): { warned: Warning[]; fired: Warning[]; wire: WireLine[] } {
...
export function fireResponse(pack: Pack, game: Game, w: Warning): WireLine[] {
  const wire: WireLine[] = [],
    L = game.ledgers,
    name = holdersOf(pack).find((h) => h.id === w.holder)?.name ?? w.holder;
  ...
  const card = (lines: WireLine[]) =>
    wire.push(...lines.map((x) => ({ ...x, kind: "card" as const })));
  switch (w.response) {
    case "riot":
      card(movePopularity(pack, game, [], -RIOT_HIT, name));
      break;
    case "refuse_levy":
      drop("treasury", LEVY_HIT);
      break;
    case "embargo":
      drop("treasury", EMBARGO_HIT);
      break;
    case "excommunicate":
      card(moveSupport(pack, game, [ownHolder(pack).id], -EXCOMMUNICATE_HIT, name));
      break;
    case "strike": {
...
const loyaltyFor = (start: Pack["starts"][number], faction: string, own: string) =>
  faction === own
    ? 100
    : (start.hostile ?? []).includes(faction)
      ? 25
      : start.coalition.includes(faction)
        ? 70
        : 0;
...
export function nationalPopularity(pack: Pack, game: Game): number {
  ...
  for (const r of pack.regions) {
    w += r.weight;
    sum += r.weight * (game.regions[r.id] ?? 50);
  }
  return w ? sum / w : 50;
}
export const popularity = (pack: Pack, game: Game) => {
  const a = nationalPopularity(pack, game);
  return a >= 55 ? "popular" : a <= 45 ? "unpopular" : "evenly split";
};
...
export function record(pack: Pack, game: Game, budget = RECORD_TOKENS): Record<string, unknown> {
  ...
  const base: Record<string, unknown> = {
    term: game.term,
    [pack.vocabulary.turn]: game.turn,
    streak: game.streak,
  ...
  const soft: [string, unknown][] = [
    ...
    [
      ...
      game.bills
        .filter((b) => b.headline)
        .slice(-RECORD_HEADLINES)
        .map((b) => b.headline!.title),
    ],
  ];
  const out = { ...base };
...
export type RunStyle = {
  line: string;
  decisive: { turn: number; line: string }[];
  grid: { ledger: Square; won?: boolean }[];
};
...
export const STYLE_LINES: Record<Square, string> = {
  ...
  chest:
    "You ruled from the private chest. What you paid for quietly moved more than the budget did.",
...
export function biggestMove(wire: WireLine[], turn: number): RunRow {
  ...
  for (const [ledger, v] of sums)
    if (v.delta > row.delta) row = { turn, ledger, delta: v.delta, cause: v.cause };
  return row;
}
...
export function runStyle(pack: Pack, game: Game): RunStyle {
  ...
  const name = (s: Square) => {
    const n = s === "quiet" ? "nothing" : (pack.constitution?.ledgers[s]?.name ?? s);
  ...
  const decisive = log
    ...
    .map((r) => ({
      turn: r.turn,
      line: `${r.cause}. ${name(r.ledger)} moved ${Math.round(r.delta)}.`,
    }));
...
export const ESCALATION_EFFECTS: Record<EscalationKey, EscalationEffects> = {
  hostile_press: {
    verdict: (pack, game) => {
      for (const r of pack.regions) bump(pack, game, r.id, -1);
    },
  },
  ...
  recession: {
    start: (pack, game) => {
      ...
      for (const p of pack.patrons.slice(0, 2)) game.patrons[p.id] = -1;
    },
  },
  scandal_season: {
    start: (_pack, game) => {
      for (const m of seeded(game, 0x5ca2, game.members, 4)) m.situation = INVESTIGATION;
    },
  },
  ...
  war_footing: {
    ...
    turn: (_pack, game) => {
      game.ledgers.authority = clamp(game.ledgers.authority - 2, 0, 200);
    },
  },
  famine: {
    start: (pack, game) => {
      game.marks.famine = seeded(game, 0xfa11, pack.regions, 10).map((r) => r.id);
      for (const id of game.marks.famine) bump(pack, game, id, -2);
    },
    turn: (pack, game) => {
      for (const id of (game.marks.famine ?? []).slice(0, 3)) bump(pack, game, id, -0.5);
    },
  },
  succession_crisis: {
    start: (pack, game) => {
      const h = game.holders[ownHolder(pack).id];
      if (h) h.support = 35;
    },
  },
  foreign_meddling: {
    start: (pack, game) => {
      game.marks.meddling = seeded(game, 0xf0e1, pack.regions, 2).map((r) => r.id);
    },
    test: (game, regions) => {
      for (const id of game.marks.meddling ?? []) {
        const r = regions.find((x) => x.id === id);
        if (r) r.p = clamp(r.p - 0.05, 0, 1);
      }
    },
  },
};
...
const first = <
  K extends "lobbyCost" | "chest" | "struckAt" | "leak" | "windowShift" | "dirLo" | "dirHi",
>(
  game: Game,
  k: K,
) =>
...
export function threshold(pack: Pack, game: Game, bill: Bill): number {
  ...
  const written =
    pack.constitution?.instruments.law.vetoes.includes("chamber_supermajority") ?? false; // R29
  return forced || veto || written || (bill.filibuster ?? 0) >= 0.5
    ? pack.chamber.supermajority
    : pack.chamber.threshold;
}
...
export const expectedYes = (whip: Record<string, number>) =>
  Object.values(whip).reduce((a, b) => a + b, 0);
...
export const FOR_AT = 2 / 3,
  AGAINST_AT = 1 / 3; // TUNE: a seat this sure either way is counted; between them it hesitates
// R30: what a hesitant faction will take for its seats on this act (acts.ts termsOf).
export interface Term {
  kind: "pledge" | "post" | "money";
  label: string;
  cost: Price;
  tag?: string;
  due?: number;
...
export function votePreview(
  ...
  const factions = pack.factions.flatMap((faction): FactionCount[] => {
    const chances = game.members
      .filter((member) => member.faction === faction.id)
      .map((member) => whip[member.id]);
    ...
    return [
      {
        ...
        for: inFavour,
        against,
        hesitant: chances.length - inFavour - against,
        reason,
      },
    ];
  });
  ...
  return {
    need: threshold(pack, game, bill),
    expected: round1(expectedYes(whip)),
    for: seatsInFavour,
    factions,
  };
}
...
export function applyVote(pack: Pack, game: Game, bill: Bill): void {
  ...
  Object.assign(bill, {
    ...
    passed,
    struck,
    vetoed: Object.values(bill.vetoes ?? {}).some((v) => v >= 0.6),
  });

  const L = game.ledgers;
  const was = L.authority;
  L.authority = clamp(
    L.authority + (passed ? LAW_PASSED : -LAW_LOST) - (struck ? STRUCK_DECREE : 0),
    0,
    200,
  );
  pushWire(game, [
    { kind: "ledger", ledger: "authority", delta: round1(L.authority - was), cause: bill.title },
  ]);
  ...
  pushWire(
    ...
    moveSupport(
      ...
      game,
      [ownHolder(pack).id],
      passed ? (yes - ownYes > ownYes ? -6 : 3) : -2,
      bill.title,
    ),
  );
  ...
  for (const [id, s] of Object.entries(bill.patrons ?? {}))
    if (id in game.patrons) game.patrons[id] = clamp(round1(game.patrons[id] + (1 - s)), -2, 2);
  for (const [id, s] of Object.entries(bill.blocs ?? {}))
  ...
  L.chest = round1(
    L.chest +
      Math.min(
        CHEST_CAP,
        Object.values(game.patrons).reduce((a, b) => a + Math.max(0, b), 0) *
          (first(game, "chest") ?? 1),
      ),
  );
  ...
  if (passed && !struck) {
    ...
    if (bill.rates?.length)
      enact(game, {
        ...
        title: bill.title,
        perTurn: bill.rates,
        repealVetoes: pack.constitution?.instruments.law.vetoes ?? ["chamber"],
        sunset: bill.sunset ?? null,
      });
  }
  ...
  for (const m of game.members) {
    ...
    if (act === "threat" && !votes[m.id]) {
      m.mood = clamp(m.mood - 0.15, -1, 1);
      line = `The ${pack.vocabulary.seat} was threatened over "${bill.title}" and still voted ${pack.vocabulary.fail}.`;
    } else if (bill.offers[m.id])
      line = votes[m.id]
        ? `Took the offer on "${bill.title}" and voted with the government.`
        : `Refused the offer on "${bill.title}".`;
    ...
    } else if (m.faction === game.faction && !votes[m.id])
      line = `Broke with their own faction and voted against "${bill.title}".`;
...
export interface InForce {
  ...
  repealVetoes: string[];
  sunset: number | null; // R29: who must agree to repeal it (none: it can be withdrawn); turns of life
}

export const inForceAge = (game: Game, law: InForce) =>
  (game.term - law.term) * TURNS_PER_TERM + (game.turn - law.turn);
...
export function applyRates(pack: Pack, game: Game): WireLine[] {
  ...
  for (const law of [...game.inForce]) {
    if (law.sunset !== null && inForceAge(game, law) >= law.sunset) {
      repeal(game, law.id);
      continue;
    }
    for (const rate of law.perTurn) {
      ...
      if (rate.ledger === "popularity") {
        wire.push(...movePopularity(pack, game, rate.id ? [rate.id] : [], rate.delta, law.title));
        continue;
      }
      if (rate.ledger === "loyalty") {
        wire.push(...moveSupport(pack, game, [ownHolder(pack).id], rate.delta, law.title));
        continue;
      }
      ...
      wire.push({
        kind: "ledger",
        ledger: rate.ledger,
        delta: round1(game.ledgers[rate.ledger] - was),
        cause: law.title,
      });
    }
  }
...
export function endTurn(pack: Pack, game: Game): TurnEnd {
  ...
  const idle = game.acts.some((a) => a.term === game.term && a.turn === game.turn)
    ...
    : moveSupport(
        pack,
        game,
        [publicHolder(pack)?.id ?? ownHolder(pack).id],
        -IDLE_COST,
        "no act was signed this turn",
      );

  // R19: the powers are held only while the turns last and the army's stance allows them.
  if (game.emergency !== null && (game.turn > game.emergency || !armyAllows(pack, game)))
    game.emergency = null;
  ...
  if (own && own.support < own.line) {
    ...
    if (!game.marks.doubled) {
      ...
      for (const m of seeded(
        game,
        0xd0b1e,
        game.members.filter((x) => !cls.has(x.seat)),
        cls.size,
      ))
        cls.add(m.seat);
  ...
  for (const h of holdersOf(pack)) {
    ...
    if (h.gives.per === "once") {
      if (gave.includes(h.id)) continue;
      game.marks.gave = [...gave, h.id];
    }
  ...
  game.quiet = [...acted, ...wire].some((w) => w.kind === "ledger" || w.kind === "support")
    ? 0
    : game.quiet + 1;
  ...
  else if (game.turn > TURNS_PER_TERM) {
    game.stage = "test";
    game.phase = "over";
  } else {
    game.phase = "draft";
    if (voted === 10) game.stage = "midterm";
  }
...
function pendingItem(
  pack: Pack,
  game: Game,
  w: { warned: Warning[]; fired: Warning[] },
  event: Event | null,
): string | null {
  const name = (id: string) => holdersOf(pack).find((h) => h.id === id)?.name ?? id;
  const open = game.warnings[0];
  if (open)
    return `${name(open.holder)} is at ${Math.round(open.number)} of a line of ${game.holders[open.holder]?.line ?? 0} and answers on turn ${open.fires}.`;
...
export const lobbyCost = (game: Game, action: LobbyAction) =>
  Math.round(LOBBY_COSTS[action] * (first(game, "lobbyCost") ?? 1));
...
export function applyLobby(
  ...
  member: Member,
  action: LobbyAction,
): { cost: number; offer: string; leak: boolean } {
...
export function applyCitizens(
  ...
  for (const [id, xs] of bloc)
    if (xs.length) game.blocs[id] = round1(clamp(mean(xs) + (game.drift[id] ?? 0), 0, 1));
  return deltas;
}
...
export const feedMemory = (region: string, reaction: string) =>
  `Constituents in ${region} were loud about the government's last notice: mostly ${reaction}.`;
...
export function applyPost(
  ...
  agree: Record<string, "government" | "rival">,
  tag: PriceTag,
): Post {
  const loud = game.stageB.loud_opposition ?? 1;
  const tally = { like: 0, boo: 0, share: 0, ignore: 0 };
  const per = new Map(
    pack.regions.map((r) => [r.id, { like: 0, boo: 0, share: 0, ignore: 0, n: 0 }]),
  );
  for (const c of pack.citizens) {
  ...
  for (const r of pack.regions) {
    ...
    const d = round1(
      clamp(((g.like + g.share) / g.n - boos - POST_BASELINE) * POST_GAIN * game.trust, -6, 6),
    );
    raw[r.id] = d;
  ...
  const post: Post = {
    ...
    boos: tally.boo,
    shares: tally.share,
    ignores: tally.ignore,
    regions,
    hot,
    targets: tag.targets ?? [],
    replies: said.replies.slice(0, 3),
    rival: said.rival,
    agree: { mine, rival: votes.length - mine },
    won: mine >= votes.length - mine,
  };
...
export function regionIntent(pack: Pack, intent: Record<string, number>): Record<string, number> {
  ...
  for (const c of pack.citizens) {
    const g = per.get(c.region);
    if (g && intent[c.id] !== undefined) {
      g.w += c.weight;
      g.s += c.weight * intent[c.id];
    }
  }
  return Object.fromEntries(
    pack.regions.map((r) => {
      const g = per.get(r.id)!;
      return [r.id, g.w ? g.s / g.w : 0.5];
    }),
  );
}
...
export function holdP(pack: Pack, game: Game, m: Member, byRegion: Record<string, number>): number {
  const base =
    0.5 * sigmoid(((game.regions[m.region] ?? 50) - 50) / 8) + 0.5 * (byRegion[m.region] ?? 0.5);
  return clamp(ownSide(pack, game, m.faction) ? base : 1 - base, 0, 1);
}

const seatShareOf = (game: Game, faction: string) =>
  game.members.filter((m) => m.faction === faction).length;
...
const winnerOf = (
  ...
  region: string,
  loser: string,
  excludeOwnSide: boolean,
): string | undefined => {
  const pool = [...pack.factions].filter(
    (f) => f.id !== loser && (!excludeOwnSide || !ownSide(pack, game, f.id)),
  );
...
export function runMidterm(pack: Pack, game: Game, intent: Record<string, number>): MidtermDraw {
  ...
  const up = cls.map((m) => ({
    ...
    memberId: m.id,
    faction: m.faction,
    p: Math.round(holdP(pack, game, m, byRegion) * 100) / 100,
  }));
  ...
  for (const m of seeded(
    game,
    0x5b1e,
    cls.filter((m) => ownSide(pack, game, m.faction)),
    n,
  )) {
  ...
  for (const m of cls) {
    ...
    if (forced) {
      lost.push({ seat: m.seat, memberId: m.id, from: m.faction, to: forced });
      continue;
    }
    ...
    if (!held)
      lost.push({
        ...
        memberId: m.id,
        from: m.faction,
        to: winnerOf(pack, game, m.region, m.faction, false)!,
      });
  }
  const lostOwn = lost.filter(
    (l) => ownSide(pack, game, l.from) && !ownSide(pack, game, l.to),
  ).length;
  return {
    up,
    forced: [...forcedTo.keys()],
    lost,
    lostOwn,
    wipeout: lostOwn * 5 >= cls.length * 2 && cls.length > 0,
  };
}
...
export function applyMidterm(pack: Pack, game: Game, draw: MidtermDraw, personas: Persona[]): void {
  ...
  for (const slot of replacements(pack, game, draw)) {
    ...
    game.members[i] = {
      ...
      bio: p?.bio ?? "",
      tell: p?.tell ?? "",
      core_issues: core.length ? core : [pack.tags[0]],
      temperament: slot.temperament,
  ...
  const seats = draw.lost.reduce(
    (a, l) => a + Number(ownSide(pack, game, l.to)) - Number(ownSide(pack, game, l.from)),
    0,
  );
  if (ch && seats)
    moveSupport(pack, game, [ch.id], (seats * 100) / pack.chamber.size, pack.vocabulary.midterm);
  game.midterm = {
    up: draw.up.map((u) => u.seat),
    lost: draw.lost,
    lostOwn: draw.lostOwn,
    wipeout: draw.wipeout,
  };
  if (draw.wipeout) {
    game.stage = "over";
    game.phase = "over";
...
export const meets = (pack: Pack, game: Game, c: Condition) =>
  c.op === "<" ? val(pack, game, c) < c.value : val(pack, game, c) > c.value;
const near = (pack: Pack, game: Game, c: Condition) =>
  meets(pack, game, c) && Math.abs(val(pack, game, c) - c.value) <= (NEAR[c.ledger] ?? 5);

const turnFor = (game: Game, s: Storylet) =>
  turnOf(s.date, game.calendar.start_date, game.calendar.unit) ?? s.turn ?? null;
...
function pick(pack: Pack, game: Game, pool: Storylet[]): Storylet | null {
  const weights = pool.map(
    (s) => s.weight * ((s.needs ?? []).some((c) => near(pack, game, c)) ? 2 : 1),
  );
...
export const foreignStorylet = (_pack: Pack, game: Game, h: Holder): Storylet => ({
  id: `foreign-${h.id}-${game.term}`,
  kind: "foreign",
  weight: 1,
  title_hint: h.responses[0],
  stances: ["Give them what they ask", "Refuse them"],
  scored: ["none"],
  needs: [],
  results: [],
  memory: null,
...
export function resolveForeign(pack: Pack, game: Game, event: Event, stance: number): WireLine[] {
  ...
  const wire = pay(
    pack,
    game,
    { authority: 0, treasury: FOREIGN_PRICE, chest: 0 },
    `${h.name} was given what it asked`,
  );
  ...
  if (h.gives && h.gives.per === "turn" && !game.inForce.some((l) => l.id === id)) {
    enact(game, {
      ...
      verb: "favour",
      title: `${h.name} pays`,
      perTurn: [{ ledger: h.gives.ledger, delta: h.gives.amount }],
      repealVetoes: [],
      sunset: null,
    });
  }
...
export function director(game: Game, pack: Pack): Event | null {
  ...
  const lo = first(game, "dirLo") ?? 30,
    hi = first(game, "dirHi") ?? 70;
  ...
  const pending = deckOf(pack, game).filter(
    (s) => s.kind === "dated" && !game.director.seen.includes(s.id),
  );
  ...
  if (clear) {
    const due = pending.find(
      (s) =>
        !s.exogenous && dueAt(game, s, 2) && (s.needs ?? []).every((c) => meets(pack, game, c)),
    );
  ...
  const forced =
    (game.turn >= CAMPAIGN_FROM &&
      game.turn <= TURNS_PER_TERM &&
      d.lastCrisis < CAMPAIGN_FROM - 1) ||
    game.quiet >= FIC_TURNS;
  ...
  if (forced) crisis = true;
    ...
    crisis = false; // never two in a row before turn 17
  else if (d.intensity < lo) crisis = roll() < 0.7;
  else if (d.intensity > hi) {
    relief = !on(game).some((e) => e.noRelief) && roll() < 0.6;
    crisis = !relief && roll() < 0.25;
  } else crisis = roll() < 0.25;
  if (!crisis && !relief) return null;

  const recent = new Set(game.events.filter((e) => game.turn - e.turn < 6).map((e) => e.id));
  const pool = deckOf(pack, game).filter(
    (s) =>
      s.kind === "generic" &&
      RELIEF.has(s.id) === relief &&
      !recent.has(s.id) &&
...
function fire(game: Game, s: Storylet, relief: boolean, holder?: string): Event {
  const kind =
    s.kind === "swan" ? "swan" : s.kind === "foreign" ? "foreign" : relief ? "relief" : "crisis";
  const e: Event = {
    id: s.id,
    turn: game.turn,
    relief,
...
export function resolveEvent(
  ...
  if (event.kind === "foreign") {
    pushWire(game, resolveForeign(pack, game, event, stance));
    return;
  }
  if (scores) {
    event.scores = scores;
    ...
    l === "approval" || l === "bloc"
      ...
      : l === "party"
        ...
        : l === "patron"
          ? (patrons ?? pub)
          : l === "seat"
            ? (chamberHolder(pack)?.id ?? null)
            : null;
  const card = deckOf(pack, game).find((s) => s.id === event.id);
  const best = (card?.results ?? [])
    .filter((e) => (e.delta ?? 0) > 0 && groupOf(e.ledger))
    .sort((a, b) => (b.delta ?? 0) - (a.delta ?? 0))[0];
  return (
    (best && groupOf(best.ledger)) ||
    (card?.scored.includes("patrons") ? patrons : null) ||
    pub ||
    ownHolder(pack).id
  );
}
...
const SEAT_MARK: Record<string, { mood: number; loyalty: number }> = {
  hostile: { mood: -0.2, loyalty: -40 },
  favor: { mood: 0.3, loyalty: 10 },
  courted: { mood: 0.2, loyalty: 10 },
  "kept-word": { mood: 0.05, loyalty: 10 },
};
function applyEffect(pack: Pack, game: Game, e: Effect, memory?: string | null) {
  ...
  const d = clamp(e.delta ?? 0, -15, 15),
    L = game.ledgers;
  switch (e.ledger) {
    case "approval":
      for (const r of pack.regions) bump(pack, game, r.id, d);
      break;
    case "capital":
      L.authority = clamp(L.authority + d, 0, 200);
      break;
    case "party":
      moveSupport(pack, game, [ownHolder(pack).id], d, "a card");
      break;
    case "chest":
      L.chest = clamp(round1(L.chest + d), 0, 9999);
      break;
    case "bloc":
      if (e.id && e.id in game.blocs) game.blocs[e.id] = clamp(game.blocs[e.id] + d, 0, 1);
      break;
    case "patron":
      if (e.id && e.id in game.patrons) game.patrons[e.id] = clamp(game.patrons[e.id] + d, -2, 2);
      break;
    case "streak":
      game.streak = Math.max(0, game.streak + d);
      game.bestStreak = Math.max(game.bestStreak, game.streak);
      break;
    case "turn":
      break; // the deck may not move the clock
    case "seat": {
      const mark = SEAT_MARK[e.set ?? ""];
      if (!mark) break;
      const pool =
        e.set === "courted" ? game.members.filter((m) => m.faction !== game.faction) : game.members;
...
const holderRows = (pack: Pack, game: Game): HolderRow[] =>
  holdersOf(pack).map((h) => ({
    id: h.id,
    name: h.name,
    weight: game.holders[h.id]?.weight ?? weightOf(pack, h.id),
...
export function termPoints(game: Game, mandate: number): TermRecord {
  ...
  const kept = p.filter((x) => x.state === "kept").length,
    broken = p.filter((x) => x.state === "broken").length;
  const points =
    passed * 10 +
    kept * 25 -
    broken * 15 +
    Math.round(mandate * 100) +
    Math.round(game.ledgers.authority / 4) +
    game.bestStreak * 5;
  return { term: game.term, passed, kept, broken, mandate, points };
}
export const score = (game: Game) =>
  Math.round(game.terms.reduce((a, t) => a + t.points * 1.5 ** (t.term - 1), 0));
...
export function endTerm(pack: Pack, game: Game, test: TestResult): void {
  ...
  if (test.early && test.won) {
    ...
    if (need > 0)
      moveSupport(
        ...
        game,
        [test.early],
        need + (chamberHolder(pack)?.id === test.early ? 50 / pack.chamber.size : 0),
        "survived the early vote",
      );
    if (game.turn > TURNS_PER_TERM) game.stage = "test";
    else {
      game.stage = game.turn === 11 ? "midterm" : "session";
      game.phase = "draft";
    } // 11: turn 10's half-term was skipped
    return;
  }
  game.test = test;
  ...
  game.phase = "over";
  game.result = {
    ending: test.won ? "reelected" : (ending(pack, game) ?? "defeated"),
    score: score(game),
  };
}
...
export function continueTerm(pack: Pack, game: Game): void {
  ...
  game.bestStreak = 0;
  game.log = [];
  game.director.intensity = 0;
  game.director.lastCrisis = -1;
  game.test = undefined;
  game.midterm = undefined;
  ...
  for (const s of pack.deck.filter(
    (x) => x.kind === "dated" && !game.director.seen.includes(x.id),
  )) {
  ...
  for (const [tag, p] of Object.entries(game.promises)) {
    if (!p.authored) {
      ...
      p.window -= TURNS_PER_TERM; // an open window carries its turns left
    else delete game.promises[tag];
  }
  const r = rng(game.seed ^ (game.term * 0x9e37));
  game.marks.midterm = [...game.members]
    .sort(() => r() - 0.5)
    .slice(0, Math.round(pack.chamber.size / 3))
    .map((m) => m.seat);
