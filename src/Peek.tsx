...
export default function Peek({
  ...
  const item: PinItem = {
    ...
    hue: hueClass(of),
    lines: rows
      .map((l) => [l.cause, sign(l.delta)] as [string, string])
      .concat([["Room to the line", String(Math.round(roomTo(value, line)))]]),
  };
  return (
    <div
      className={`peek ${hueClass(of)}`}
      role="group"
      aria-label={`${name}, this turn`}
      onPointerEnter={(e) => {
        (e.currentTarget as HTMLElement).dataset.stick = "1";
      }}
      onPointerLeave={(e) => {
        delete (e.currentTarget as HTMLElement).dataset.stick;
        onClose();
      }}
    >
      <div>
        ...
        <ul className="flow">
          {rows.length ? (
            rows.map((l, i) => (
              <li key={i}>
                <span>{l.cause}</span>
                <b className={`num ${l.delta < 0 ? "neg" : ""}`}>{sign(l.delta)}</b>
              </li>
            ))
          ) : (
            <li>
              <span>Nothing moved it this turn.</span>
              <b className="num">0</b>
            </li>
          )}
        </ul>
      ...
      <div>
        ...
        <div className="small muted">
          Fails at <b className="num">{line}</b>
          {cause ? (
            <>
              <br />
              <b>{cause}</b>
      ...
      <div>
        ...
        <div className="acts2">
          <button className="btn sm" onClick={() => onPin(item)}>
            Pin
          </button>
