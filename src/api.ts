...
export type GameView = Omit<
  Game,
  "pack" | "director" | "members" | "bills" | "ledgers" | "holders" | "extra" | "calls"
> & {
...
export type FrameFragment = {
  ...
  place: string;
  description: string;
  theme: Partial<Pack["theme"]>;
  factions: { id: string; name: string; short: string; color: string }[];
  problems?: string[];
...
export const api = {
  ...
  playDaily: (faction: string, promises: number[], platform?: string) =>
    call<GameView>("/games", { mode: "daily", faction, promises, platform }),
  ...
  resolve: (g: GameView, i: number, stance: number) =>
    call<GameView>(`/games/${g.id}/events/${i}`, { turn: g.turn, stance }),
