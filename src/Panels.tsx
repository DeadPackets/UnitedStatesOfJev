...
export function Country({ game }: { game: GameView }) {
  ...
  return (
    <>
      ...
      {low ? (
        <p className="note">
          {low.name} at {Math.round(game.regions[low.id] ?? 50)} is the drag.
        </p>
      ) : null}
    </>
  );
    ...
    <>
      <ul className="causes" aria-label="The room">
        {game.holders.map((x) => (
          <li key={x.id}>
            <button
              className="rowbtn"
              aria-current={x.id === selected}
              onClick={() => onPick(x.id)}
            >
              <b className="num">{Math.round(x.support)}</b>
              <span>
                {x.name}, {x.where === "abroad" ? "abroad" : "at home"}, line {x.line}
              </span>
            </button>
          </li>
        ))}
      ...
      {h ? (
        <div className="panel">
          ...
          <p className="small">
            Weight {h.weight.toFixed(2)}. Moved by{" "}
            {h.levers.map((l) => game.instruments[l]?.name ?? l).join(", ")}.
          </p>
          {members === "blocs" || members === "citizens" ? (
            <ul className="causes" aria-label="Its people">
              {game.pack.blocs.map((b) => (
                <li key={b.id}>
                  <b className="num">{Math.round((game.blocs[b.id] ?? 0.5) * 100)}</b>
                  <span>{b.name}</span>
                </li>
              ))}
            </ul>
          ...
          <button
            ...
            onClick={() =>
              onPin({
                ...
                hue: "",
                lines: [
                  ["Support", String(Math.round(h.support))],
                  ["Line", String(h.line)],
...
export function RecordTab({ game }: { game: GameView }) {
  ...
  const left = (f: GameView["inForce"][number]) =>
    f.sunset! - ((game.term - f.term) * game.turnsPerTerm + (game.turn - f.turn));
  return (
    <>
      <div className="panel">
          ...
          {game.inForce.length ? (
            game.inForce.map((f) => (
              <li key={f.id}>
                ...
                <span>
                  {f.title}, each {v.turn}, repeal needs{" "}
                  {f.repealVetoes.join(", ").replace(/_/g, " ") || "none"}
                  {f.sunset ? `, lapses in ${Math.max(0, left(f))}` : ""}
                </span>
              </li>
            ))
          ) : (
            <li>
              <span>Nothing is in force yet.</span>
            </li>
          )}
        </ul>
      </div>
      <div className="panel">
        ...
        <div className="pledges">
          {promises.map((p) => (
            <span
              key={p.label}
              className={`stampsm tiny ${p.state === "kept" ? "pass" : p.state === "broken" ? "fail" : "wait"}`}
            >
              {p.label}
              {p.state === "pending" ? ` · ${Math.max(0, p.window - game.turn)}` : ""}
            </span>
          ))}
        </div>
      </div>
      ...
      <div className="panel">
        ...
        <p className="small">
          {difficulty(game.shortfall)}.{" "}
          {game.shortfall > 0 ? (
            <>
              You need <span className="num">{game.shortfall}</span> more votes than you hold
            </>
          ) : (
            "You hold enough votes alone"
          )}
          {game.handicap ? (
            <>
              , and you opened with <span className="num">{game.handicap}</span> less authority
            </>
          ) : null}
          .
        </p>
