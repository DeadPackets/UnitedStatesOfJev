  ...
  useEffect(() => {
    ...
    const watch = new MutationObserver(() => {
      ...
      timer.current = setTimeout(() => {
        ...
        if (!touched && document.activeElement === document.body)
          document.querySelector<HTMLElement>("[data-primary]")?.focus();
        onClose();
...
function Poster({
  ...
  return (
    <dialog
      ...
      onClick={(e) => {
        if (!block && e.target === ref.current) dismiss();
      }}
    >
      {children(dismiss)}
    </dialog>
...
const KICKER: Record<CardKind, string> = {
  crisis: "A crisis",
  foreign: "A move abroad",
  swan: "Out of nowhere",
  warning: "A warning",
  escalation: "This term brings",
};
...
type Props = {
  ...
  event: ViewEvent;
  kind: CardKind;
  holders: { id: string; name: string; stance: number }[];
  turn: number;
  busy: boolean;
  onStance: (i: number) => void;
  onClose: () => void;
};
      ...
      {(dismiss) => (
        <>
          <div className="kicker">
            {KICKER[kind]} · {pack.vocabulary.turn} {turn}
          </div>
          <h2>{title}</h2>
          ...
          {!answered ? (
            <div className="amend">
              {stances.map((s, i) => (
                <button key={i} className="opt2" disabled={busy} onClick={() => onStance(i)}>
                  <b>{s}</b>
                </button>
              ))}
            </div>
            ...
            <>
              ...
              <button ref={done} className="btn" onClick={dismiss}>
                Close the card
              </button>
            </>
          )}
...
export function WarningCard({
  ...
  pack: GamePack;
  holder: { name: string; line: number };
  warning: { response: string; fires: number; number: number };
  busy: boolean;
  onHold: () => void;
  onClose: () => void;
}) {
  const RESPONSE: Record<string, string> = {
    early_test: "call the test early",
    coup: "end your rule",
    strike: "strike your last decree",
    refuse_levy: "refuse the next levy",
    riot: "riot",
    excommunicate: "excommunicate you",
    embargo: "close the purse",
  };
  return (
    <Poster label={`${holder.name} warns you`} block={false} onClose={onClose}>
      {(dismiss) => (
        <>
          <div className="kicker">
            A warning · {pack.vocabulary.turn} {warning.fires}
          </div>
          <h2>
            {holder.name} is at {Math.round(warning.number)}.
          </h2>
          <p>
            Their line is {holder.line}. If they are still over it at {pack.vocabulary.turn}{" "}
            {warning.fires} they will {RESPONSE[warning.response] ?? "act"}.
          </p>
          <div className="amend">
            <button className="opt2" disabled={busy} onClick={onHold}>
              <b>Hold</b>
              <span className="small muted">Leave it and take the risk.</span>
            </button>
            <button className="opt2" disabled={busy} onClick={dismiss}>
              <b>Go and work on them</b>
              <span className="small muted">Close this and spend the turn easing them.</span>
            </button>
          </div>
        </>
      )}
