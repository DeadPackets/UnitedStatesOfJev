...
export function orderMembers(
  ...
  for (const m of members) {
    const b = blocks.get(m.faction);
    if (b) b.push(m);
    else blocks.set(m.faction, [m]);
  }
  const rank = (id: string) => (id === own ? 0 : coalition.includes(id) ? 1 : 2);
  return [...blocks.keys()]
    .sort(
      (a, b) =>
        rank(a) - rank(b) || blocks.get(b)!.length - blocks.get(a)!.length || a.localeCompare(b),
    )
    .flatMap((id) =>
      whip
        ? [...blocks.get(id)!].sort((x, y) => (whip[y.id] ?? 0.5) - (whip[x.id] ?? 0.5))
        : blocks.get(id)!,
    );
}
...
export const Chamber = memo(
  forwardRef<RollHandle, ChamberProps>(function Chamber(
    { pack, members, own, coalition, whip, votes, selected, onPick = NOOP, hot, pulse, rolling },
    ref,
  ) {
    const ordered = useMemo(
      () => orderMembers(members, own, coalition, whip),
      [members, own, coalition, whip],
    );
    const seats = useMemo(
      () => points(pack.theme.layout, members.length),
      [pack.theme.layout, members.length],
    );
      ...
      () => ({
        roll(result, onCount, onDone, needed) {
          ...
          const show = () => {
            ...
            if (result[id]) yes++;
            if (g) g.dataset.vote = result[id] ? "yes" : "no";
            onCount(yes);
          };
          const tick = (now: number) => {
            ...
            if (shown < ids.length) raf.current = requestAnimationFrame(tick);
            else onDone();
          };
    ...
    const seatGroups = useMemo(
      () =>
        ordered.map((m, i) => {
          ...
          const label =
            `${m.name}, ${vocab.member}, ${f?.name ?? m.faction}, ${regions.get(m.region) ?? m.region}` +
          ...
          return (
            ...
            <g
              key={i}
              ref={(el) => {
                if (el) groups.current.set(m.id, el);
                else groups.current.delete(m.id);
              }}
              className="seatg"
              role={onPick === NOOP ? "img" : "button"}
              tabIndex={0}
              aria-label={label}
              style={
                {
            ...
            >
              ...
              <circle
                className={`seatc ${pulse === m.id ? "pulse" : ""}`}
                cx={s.x}
                cy={s.y}
                r={r}
                fill={f ? fillFor(f, scope) : "var(--ink)"}
                stroke={color}
                strokeWidth={selected === m.id ? 3 : 1.4}
                opacity={p === undefined ? 1 : 0.4 + 0.6 * p}
              />
              {face >= 5 ? (
                <g opacity={p === undefined ? 1 : 0.4 + 0.6 * p}>
                  ...
                  {face >= 6 ? (
                    <text
                      ...
                      y={s.y + face * 0.36}
                      textAnchor="middle"
                      style={{ fill: color, fontSize: face }}
                    >
                      {initials(m.name)}
                    </text>
                  ) : null}
    ...
    return (
      <svg
        className={`hemi seats ${whip || votes ? "" : "gather"}`}
      ...
      >
        ...
        {h
          ? (() => {
              const s = seats[hover!],
                f = factions.get(h.faction),
                p = whip?.[h.id];
              const sub =
                `${vocab.member} · ${f?.short ?? h.faction} · ${regions.get(h.region) ?? h.region}` +
                (p === undefined ? "" : ` · ${Math.round(p * 100)}%`);
              const w = Math.min(560, Math.max(h.name.length * 8.6, sub.length * 6.6) + 62);
              const lx = Math.min(Math.max(s.x - w / 2, 4), BOX.w - 4 - w),
                ly = Math.max(s.y - r - 56, 4);
              return (
                // the rise keyframe ends on `transform: none`, which would beat a transform attribute on the same node
                <g className="seatlabel" transform={`translate(${lx} ${ly})`}>
                  <g className="rise">
                    ...
                    <text
                      ...
                      y={28}
                      textAnchor="middle"
                      style={{ fill: f?.color, fontSize: 13 }}
                    >
                      {initials(h.name)}
                    </text>
                    <text x={54} y={20}>
