...
function Headline({ bill }: { bill: Bill }) {
  return <div key={bill.id} className="card headline rise" style={{ animationDelay: "120ms" }}><div className="kicker">Wire</div><h3>{bill.headline!.title}</h3><p className="muted small" style={{ margin: "6px 0 0" }}>{bill.headline!.lede}</p></div>;
}
...
export default function Chamber({ game, act, busy, onQuit }: { game: GameView; act: Act; busy: boolean; onQuit: () => void }) {
  ...
  useEffect(() => {
    ...
    const step = () => {
      if (i >= ids.length) { setRevealed(null); setRollYes(null); done(); return; }
    ...
    <main className="chamber" onPointerDown={sound.unlock}>
      ...
      <section className="stage" aria-label="Chamber floor">
        {showMap ? <div className="rise"><Map approval={game.approval} /></div>
          : <Hemicycle seated={game.seated} bill={bill} party={game.settings.party} onPick={(s) => { if (!rolling) setPick(s); }} selected={sel?.id} revealed={revealed} pulse={pulse} hot={step?.id === "lobby" ? weakest?.id : undefined} />}
        {!bill ? <p className="prompt rise" style={{ margin: "0 auto" }}>{agenda ? "The Senate is seated. Send the next bill." : "The Senate is seated. Write a bill."}</p> : (
          <>
            <div className={`count ${crossed ? "bounce" : ""}`}>
              {voted ? <Num value={shownYes} className={`n ${!rolling ? (bill.passed ? "pass" : "fail") : ""}`} /> : whipped ? <Num value={exp} decimals={1} className="n" /> : <span className="n muted">—</span>}
            ...
            <div className="whipbar" role="meter" aria-valuemin={0} aria-valuemax={100} aria-valuenow={voted ? shownYes : exp} aria-label="Yes votes">
              <div className={`fill ${voted && !rolling ? (bill.passed ? "pass" : "fail") : ""}`} style={{ width: `${voted ? shownYes : exp}%` }} />
      ...
      <aside className="rail" aria-label="Desk">
        ...
        {!bill ? (
          <div key="pad" className="card billpad rise" data-tour="billpad">
            ...
            <div className="actions"><button className={`btn hot ${busy ? "busy" : ""}`} disabled={busy || (!agenda && text.trim().length < 12)} onClick={draft}>{busy ? "Drafting" : "Send to the floor"}</button></div>
          </div>
        ) : (
          <div key={`bill-${bill.id}-${bill.title}`} className="card billcard rise">
            ...
            <div className="tags">{bill.tags.map((t, i) => <span key={t} className="chip rise" style={{ animationDelay: `${120 + i * 40}ms` }}>{t}</span>)}</div>
            {voted && !rolling ? <div style={{ marginTop: 12 }}><span className={`stamp stampin ${bill.passed ? "pass" : "fail"}`}>{bill.passed ? "Passed" : "Failed"} {yes}–{100 - yes}</span></div> : null}
            <div className="actions">
              {!whipped ? <button className={`btn hot ${busy ? "busy" : ""}`} data-tour="whip" disabled={busy} onClick={() => act(() => api.whip(game))}>{busy ? "Counting" : "Whip count"}</button> : null}
              {whipped && !voted ? <>
                <button className={`btn hot ${busy ? "busy" : ""}`} data-tour="vote" data-tour-hot={step?.id === "vote"} disabled={busy} onClick={() => act(() => api.vote(game))}>{busy ? "Voting" : "Call the vote"}</button>
              ...
              {voted && !rolling ? <button className="btn hot" disabled={busy} onClick={() => setDismissed(game.turn)}>Next bill</button> : null}
            </div>
            {amendments && amendments.length > 0 && !voted ? <div className="amend"><div className="kicker">Pick an amendment</div>
              {amendments.map((a, i) => <button key={i} className="opt" disabled={busy} onClick={() => act(() => api.adopt(game, i))}><b>{a.title}</b><span className="small muted">{a.summary}</span><span className="small num">expected yes {a.expected.toFixed(1)}</span></button>)}</div> : null}
          </div>
        )}
