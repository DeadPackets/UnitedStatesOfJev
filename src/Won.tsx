    ...
    <main className="over stagger press">
      ...
      {t ? (
        <div className="ledger" style={{ "--i": 3 } as any}>
          <div>
            <div className="k">Score, term {t.term}</div>
            <div className="v">
              <Num value={t.points} />
            </div>
          </div>
          <div>
            <div className="k">{pack.constitution?.retention.name ?? pack.test.name}</div>
            <div className="v">
              <Num value={Math.round(t.mandate * 100)} /> of{" "}
              <span className="num">{Math.round(game.bar * 100)}</span>
            </div>
          </div>
          <div>
            <div className="k">{v.pass}</div>
            <div className="v">
              <Num value={t.passed} />
            </div>
          </div>
      ...
      <div className="panel escal" style={{ "--i": 4 } as any}>
        <div className="kicker">
          Another term asks for {Math.round(barAt(pack, game.term + 1) * 100)} of the room
        </div>
        {next.length ? (
          next.map((e) => (
            <div key={e!.key}>
      ...
      <div className="row" style={{ "--i": 5 } as any}>
        <button
          ...
          disabled={busy}
          onClick={() => act(() => api.continue(game))}
        >
          Another term
        </button>
        <button className="btn ghost" disabled={busy} onClick={() => act(() => api.stop(game))}>
          Stop here
        </button>
        <span className="small muted">
          Another term carries the acts in force and the grudges. Stopping here banks the score and
