...
export const sound = {
  get muted() { try { return localStorage.getItem("usoj:mute") === "1"; } catch { return false; } },
  set muted(v: boolean) { try { localStorage.setItem("usoj:mute", v ? "1" : "0"); } catch {} },
  unlock() { try { const c = get(); if (c.state === "suspended") c.resume(); } catch {} },
  play(name: "tick" | "chime" | "click" | "gavel" | "thud" | "slide", opt: { pitch?: number } = {}) {
    if (sound.muted) return;
    try { CUES[name](get(), opt.pitch ?? 0); } catch {}
  },
};

function tone(c: AudioContext, freq: number, dur: number, type: OscillatorType, gain = 0.08, at = 0) {
...
const CUES: Record<string, (c: AudioContext, pitch: number) => void> = {
  ...
  chime: (c) => { tone(c, 660, 0.25, "sine", 0.06); tone(c, 990, 0.35, "sine", 0.05, 0.09); },
  click: (c) => tone(c, 1400, 0.03, "triangle", 0.05),
  gavel: (c) => { noise(c, 0.09, 900, 0.9); tone(c, 180, 0.12, "sine", 0.15); noise(c, 0.09, 900, 0.7, 0.16); tone(c, 170, 0.12, "sine", 0.12, 0.16); },
  thud: (c) => { tone(c, 90, 0.35, "sine", 0.18); noise(c, 0.2, 300, 0.4); },
