...
const LEVY =
  "Levy a duty of one part in twenty on grain and wine brought into the ports, paid into the treasury.";
...
async function api(path: string, body?: unknown): Promise<GameView> {
  for (let attempt = 0; ; attempt++) {
    ...
    if (r.status >= 500) {
      console.error(`FAIL ${r.status} ${path}: ${text.slice(0, 300)}`);
      process.exit(1);
    }
...
console.log(
  `${g.pack.title}: ${g.seatTitle}, ${faction}. code ${g.code} calendar ${g.calendar.start_date}/${g.calendar.unit}`,
);
console.log(
  `${g.pack.chamber.size} ${V.member}s, ${V.bill} needs ${g.pack.chamber.threshold}, bar ${g.bar.toFixed(2)}` +
    `, room: ${g.holders.map((h) => `${h.name} (${h.weight.toFixed(2)})`).join(", ")}`,
);
console.log(
  `${V.promise}s: ${Object.values(g.promises)
    .map((p) => p.label)
    .join(" / ")}\n`,
);
...
const pays = (c: { authority: number; treasury: number; chest: number }) =>
  c.authority <= g.ledgers.authority &&
  c.treasury <= g.ledgers.treasury &&
  c.chest <= g.ledgers.chest;
...
while (g.stage === "session" || g.stage === "midterm" || g.stage === "test") {
  if (g.stage === "test") {
    ...
    if (t) {
      for (const h of t.holders)
        console.log(
          `  ${h.counted ? "x" : " "} ${h.name.padEnd(24)} weight ${h.weight.toFixed(2)} support ${h.support.toFixed(1)}`,
        );
      console.log(
        `  mandate ${t.mandate.toFixed(3)} against a bar of ${t.bar.toFixed(3)} -> ${t.won ? "WON" : "LOST"}`,
      );
  ...
  if (g.stage === "midterm") {
    ...
    const m = g.midterm!;
    console.log(
      `   ${V.midterm}: ${m.up.length} up, ${m.lost.length} lost (${m.lostOwn} own side)${m.wipeout ? " (wipeout)" : ""} in ${ms(m0)} | ${m.headline?.title ?? "(no headline)"}`,
    );
  ...
  if (card >= 0 && afford(2)) {
    ...
    console.log(
      `     ${e.kind ?? "crisis"} ${e.id} "${e.card?.title ?? ""}" -> "${e.stances[0]}": ${g.events[card].outcome ?? "(no line)"}`,
    );
  }
  ...
  if (afford(2) && open("law")) {
    g = await api(`/games/${g.id}/acts/price`, {
      turn,
      verb: "law",
      text: g.ledgers.treasury < 5 ? LEVY : TEXTS[(turn - 1) % TEXTS.length],
    });
    if (g.refusal) refusals.push(`t${turn} ${g.refusal.test}: ${g.refusal.line}`);
    else if (!pays(g.tag!.charge)) {
      const c = g.tag!.charge;
      console.log(
        `${String(turn).padStart(2)} law "${g.tag!.title}" left on the desk: it costs ${c.authority}a/${c.treasury}t/${c.chest}c`,
      );
    } else {
      const tag = g.tag!;
      console.log(
        `${String(turn).padStart(2)} ${tag.verb} "${tag.title}" cost ${tag.charge.authority}a/${tag.charge.treasury}t/${tag.charge.chest}c` +
      ...
      console.log(
        `     band ${bill.band?.[0]} to ${bill.band?.[1]} of ${bill.needed}, expected ${bill.expected}`,
      );
      tabled = true;
    }
  }
  ...
  if (tabled && turn === 1 && afford(1)) {
    ...
    g = await api(`/games/${g.id}/bills/${turn}/lobby`, {
      turn,
      memberId: soft.id,
      action: "pork",
    });
  ...
  if (tabled && turn === 2 && afford(3)) {
    ...
    console.log(
      `     amendments: ${rows.map((a) => `${a.title} (${a.expected})`).join(" | ") || "none"}`,
    );
    if (rows.length) {
  ...
  if (tabled && afford(1)) {
    ...
    console.log(
      `     ${voted.passed ? (voted.struck ? "STRUCK" : V.pass) : V.fail} ${voted.yes}/${voted.threshold} | ${voted.headline?.title ?? "(no headline)"}`,
    );
  }
  ...
  if (afford(3) && open("proclaim")) {
    g = await api(`/games/${g.id}/acts/price`, {
      turn,
      verb: "proclaim",
      text: POSTS[(turn - 1) % POSTS.length],
    });
    if (g.refusal) refusals.push(`t${turn} ${g.refusal.test}: ${g.refusal.line}`);
    else if (pays(g.tag!.charge)) {
      g = await api(`/games/${g.id}/acts`, { turn });
      ...
      console.log(
        `     ${V.post}: ${p.likes} like ${p.boos} boo ${p.shares} share ${p.ignores} ignore | duel ${p.agree.mine}/${p.agree.rival} -> ${p.won ? "won" : "lost"}`,
      );
    }
  }
  ...
  const L = g.ledgers;
  const nat =
    g.pack.regions.reduce((a, r) => a + r.weight * (g.regions[r.id] ?? 50), 0) /
    g.pack.regions.reduce((a, r) => a + r.weight, 0);
  console.log(
    `     ledgers treasury ${L.treasury} authority ${L.authority} chest ${L.chest} popularity ${nat.toFixed(1)}` +
      ` | calls ${g.calls.spent}/${g.calls.cap} | room ${g.holders.map((h) => `${h.id} ${Math.round(h.support)}/${h.line}`).join(" ")}`,
  );
  for (const w of g.wire.slice(0, 4))
    console.log(
      `     wire ${w.kind} ${w.ledger ?? w.id ?? ""} ${w.delta > 0 ? "+" : ""}${w.delta} (${w.cause})`,
    );
  if (g.pending) console.log(`     next: ${g.pending}`);
  timings.push(performance.now() - t0);
  if (turn === g.turn) {
    console.error(`FAIL: ${V.turn} ${turn} did not advance`);
    process.exit(1);
  }
}
...
if (term)
  console.log(
    `term ${term.term}: passed ${term.passed} kept ${term.kept} broken ${term.broken} mandate ${term.mandate.toFixed(3)} points ${term.points}`,
  );
...
if (g.stage === "won") {
  ...
  if (g.term !== 2 || g.turn !== 1 || g.stage !== "session") {
    console.error("FAIL: continue did not open term 2");
    process.exit(1);
  }
}
console.log(`\n${calls} requests, ${ms(started)} total`);
if (performance.now() - started > 480_000) {
  console.error("FAIL: over 8 minutes");
  process.exit(1);
}
