...
const page = await fetchWikipedia("en", "Julius Caesar", ["assassination", "dictatorship", "civil war", "early life"]);
...
for (const name of ["Julius Caesar", "Mark Antony", "Cleopatra", "Marcus Junius Brutus"]) {
  ...
  console.log(name, "->", p ? `${p.qid} born ${p.born} died ${p.died ?? "?"}` : "not found / rejected");
}
