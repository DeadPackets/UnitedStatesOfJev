...
const schema = z.toJSONSchema(
  z.object({ title: z.string(), summary: z.string(), tags: z.array(z.string()) }),
);
const text =
  "Give every public school teacher a $10,000 raise, paid for by closing the carried-interest loophole.";
...
for (const [name, extra] of Object.entries(variants)) {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    try {
      const r = await post(env, "chat/completions", {
        ...
        messages: [
          {
            role: "system",
            content:
              "Turn the proposal into a bill: title (3-7 words ending in Act), summary (<=60 words), tags (1-4).",
          },
          { role: "user", content: text },
        ],
        response_format: {
          type: "json_schema",
          json_schema: { name: "bill", strict: true, schema },
        },
      });
      provider = r.provider ?? "";
      if (!r.choices?.[0]?.message?.content) err = JSON.stringify(r).slice(0, 120);
    } catch (e) {
