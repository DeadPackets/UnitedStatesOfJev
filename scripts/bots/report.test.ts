...
test("a perfect forecast scores zero and a backwards one scores one", () => {
  expect(brier([turn({ whip: { expected: 60, needed: 31, yes: 60, size: 60 } })])).toBeCloseTo(
    0,
    5,
  );
...
test("win rates are the first term only, per policy, in points", () => {
  const rates = winRates([
    run({ policy: "greedy", term1Won: true }),
    run({ policy: "greedy", term1Won: false }),
...
test("the draw share is the spread inside one policy over the spread across all of them", () => {
  const same = [
    run({ policy: "a", mandate: 0.5 }),
    run({ policy: "a", mandate: 0.5 }),
    run({ policy: "b", mandate: 0.7 }),
    run({ policy: "b", mandate: 0.7 }),
  ];
  ...
  const mixed = [
    run({ policy: "a", mandate: 0.2 }),
    run({ policy: "a", mandate: 0.4 }),
    run({ policy: "b", mandate: 0.6 }),
    run({ policy: "b", mandate: 0.8 }),
  ];
...
test("a style whose losses are its own failure passes R23's second half", () => {
  expect(
    ownFailure([
      run({ policy: "strongman", won: false, ending: "coup" }),
      run({ policy: "strongman", won: false, ending: "coup" }),
    ]).strongman,
  ).toBe(true);
  expect(
    ownFailure([
      run({ policy: "strongman", won: false, ending: "defeated" }),
      run({ policy: "strongman", won: false, ending: "defeated" }),
    ]).strongman,
  ).toBe(false);
});
...
test("the report fails when a check misses its target", () => {
  const bad = report(
    [run({ policy: "greedy", term1Won: true }), run({ policy: "broker", term1Won: true })],
    [turn()],
  );
