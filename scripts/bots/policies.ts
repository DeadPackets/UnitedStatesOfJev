...
export type Policy = {
  name: string;
  style: boolean;
  acts(g: GameView, rnd: () => number): BotAct[];
};
...
const can = (g: GameView, v: Verb) => {
  const i = (
    g as never as {
      instruments?: Partial<Record<Verb, { available: boolean; affordable?: boolean }>>;
    }
  ).instruments?.[v];
  return !!i?.available && i.affordable !== false;
};
const holders = (g: GameView) =>
  (
    g as never as {
      holders?: { id: string; name: string; support: number; line: number; weight: number }[];
    }
  ).holders ?? [];
const nearestLine = (g: GameView) =>
  holders(g)
    .slice()
    .sort((a, b) => a.support - a.line - (b.support - b.line))[0];
const heaviest = (g: GameView) =>
  holders(g)
    .slice()
...
const regionName = (g: GameView, id: string | undefined) =>
  g.pack.regions.find((r) => r.id === id)?.name ?? "the region that needs it most";
...
export const POLICIES: Policy[] = [
  {
    ...
    acts(g, rnd) {
      const open: Verb[] = (
        ["decree", "law", "appoint", "spend", "proclaim", "favour", "force"] as Verb[]
      ).filter((v) => can(g, v));
      const verb = open[Math.floor(rnd() * open.length)] ?? "proclaim";
      const text =
        verb === "proclaim"
          ? pick(POSTS, Math.floor(rnd() * 3))
          : verb === "law"
            ? pick(WORKS, Math.floor(rnd() * 3))
            : pick(ORDERS, Math.floor(rnd() * 3));
      return [{ verb, text }];
    },
  },
  {
    ...
    acts(g) {
      ...
      if (can(g, "spend") && region)
        out.push({
          verb: "spend",
          text: `Send relief to ${regionName(g, region)}, where the need is plainest.`,
        });
      return out.length
        ? out
        : [{ verb: firstAvailable(g, ["law", "decree"]) ?? "law", text: pick(WORKS, g.turn) }];
    },
  },
  {
    name: "strongman",
    ...
    acts(g) {
      const out: BotAct[] = [];
      if (can(g, "appoint") && g.turn === 1)
        out.push({
          verb: "appoint",
          text: "Put a loyal officer at the head of the security service.",
        });
      ...
      if (over && can(g, "force"))
        out.push({
          verb: "force",
          text: `Put a curfew on the districts where ${over.name} will not settle.`,
        });
      return out.length
        ? out
        : [{ verb: firstAvailable(g, ["law", "proclaim"]) ?? "law", text: pick(WORKS, g.turn) }];
    },
  },
  {
    name: "populist",
    ...
    acts(g) {
      ...
      if (can(g, "spend") && region)
        out.push({
          verb: "spend",
          text: `Pay relief straight to the households of ${regionName(g, region)}.`,
        });
      if (can(g, "decree") && g.turn % 4 === 0)
        out.push({ verb: "decree", text: pick(ORDERS, g.turn) });
  ...
  },
  {
    ...
    acts(g) {
      ...
      if (near && member && can(g, "favour"))
        out.push({
          verb: "favour",
          text: `A place at the table and a share of the works, for a vote with ${near.name}.`,
          memberId: member,
        });
      if (can(g, "law")) out.push({ verb: "law", text: pick(WORKS, g.turn) });
      const big = heaviest(g);
      if (big && can(g, "appoint") && g.turn % 6 === 0)
        out.push({
          verb: "appoint",
          text: `Give the post to ${big.name}, which holds the most weight of any of them.`,
        });
  ...
  },
  {
    ...
    acts(g) {
      const pending = Object.entries(g.promises ?? {}).find(
        ([, p]) => (p as { state: string }).state === "pending",
      );
      const out: BotAct[] = [];
      if (can(g, "law"))
        out.push({
          verb: "law",
          text: pending
            ? `Write into law what was promised: ${(pending[1] as { label: string }).label}.`
            : pick(WORKS, g.turn),
        });
      if (can(g, "proclaim")) out.push({ verb: "proclaim", text: pick(POSTS, g.turn) });
      return out.length
        ...
        : [
            {
              verb: "spend",
              text: `Send relief to ${regionName(g, weakestRegion(g))}, where the need is plainest.`,
            },
          ];
    },
  },
