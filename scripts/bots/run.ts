...
export const TERM_USD = 0.1; // TUNE: measured, one 20 turn v4 term on a 72 seat pack, 2026-09-23
...
const holdersOf = (g: GameView) =>
  ((g as never as { holders?: TurnLog["holders"] }).holders ?? []).map((h) => ({
    id: h.id,
    support: h.support,
    line: h.line,
    weight: h.weight,
...
const priceOf = (g: GameView, verb: string): Record<string, number> => {
  const p =
    (g as never as { instruments?: Record<string, { price?: Record<string, number> }> })
      .instruments?.[verb]?.price ?? {};
...
export async function runTerm(
  ...
  while (g.stage === "session" || g.stage === "midterm") {
    ...
    if (performance.now() - started > TERM_MS)
      throw new Error(`the term took over ${Math.round(TERM_MS / 60_000)} minutes`);
    if (g.stage === "midterm") {
      g = await bot.midterm(g);
      continue;
    }
  ...
  if (last?.run === run) {
    last.jev = {
      ...last.jev,
      tokens: last.jev.tokens + u.tokens,
...
if (planned > MAX_TERMS || planned * TERM_USD > BUDGET_USD) {
  console.error(
    `FAIL: ${planned} terms is about $${(planned * TERM_USD).toFixed(2)}, over the cap of ${MAX_TERMS} terms / $${BUDGET_USD}.`,
  );
  process.exit(1);
}
console.log(
  `${chosen.length} policies x ${seeds} seeds x up to ${maxTerms} terms = at most ${planned} terms, about $${(planned * TERM_USD).toFixed(2)}`,
);
...
for (const policy of chosen) {
  for (let s = 0; s < seeds; s++) {
    ...
    console.log(
      `${run}: ${terms} term(s), ${g.result?.ending ?? "unfinished"}, mandate ${t?.mandate?.toFixed(3) ?? "n/a"} vs bar ${t?.bar?.toFixed(3) ?? "n/a"}, ${bot.calls} calls`,
    );
    if (spent > BUDGET_USD) {
      console.error(`stopping: $${spent.toFixed(2)} spent`);
      break;
    }
  }
