...
if (mode === "export") {
  ...
  const raw =
    await $`bunx wrangler d1 execute usoj --local --config ./wrangler.bots.jsonc --json --command ${sql}`.text();
  ...
  console.log(
    `  jev ${rows.filter((r) => r.kind === "jev").length}, luna ${rows.filter((r) => r.kind === "luna").length}`,
  );
  process.exit(0);
}
...
const key =
  ...
  (await readFile(".dev.vars", "utf8")).match(/OPENROUTER_API_KEY=(.*)/)?.[1]?.trim();
if (!key) {
  console.error("FAIL: set OPENROUTER_API_KEY or put it in .dev.vars");
  process.exit(1);
}

// A Luna answer is prose with no distribution to compare, so replaying it would spend money and measure nothing.
const rows: Row[] = (await readFile(file, "utf8"))
  .trim()
  .split("\n")
  .filter(Boolean)
  .map((l) => JSON.parse(l))
  .filter((r: Row) => r.kind === "jev");
if (rows.length > GOLDEN_N) {
  console.error(
    `FAIL: ${rows.length} prompts in ${file}, over the cap of ${GOLDEN_N}. Split the file and replay one part.`,
  );
  process.exit(1);
}
...
for (const row of rows) {
  ...
  const now = probs(await r.json()),
    then = probs(JSON.parse(row.answer));
...
console.log(
  mean < GOLDEN_SHIFT
    ? "pass: the models grade the frozen set as they did"
    : `FAIL: over ${GOLDEN_SHIFT}. Treat this as a balance change and re-measure.`,
);
