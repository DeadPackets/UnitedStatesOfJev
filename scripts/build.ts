...
if (!post.ok) {
  console.error(post.status, await post.text());
  process.exit(1);
}
...
for (;;) {
  ...
  if (row.status === "failed") {
    console.log(`  ${step} failed: ${row.error}`);
    process.exit(1);
  }
  if (row.status === "ready") {
    ...
    console.log(
      `chamber ${p.chamber.size}, threshold ${p.chamber.threshold}, ${p.tags.length} tags`,
    );
    console.log(
      "factions: " +
        p.factions
          .map((f: any) => `${f.name} (${p.members.filter((m: any) => m.faction === f.id).length})`)
          .join(", "),
    );
    console.log(
      "problems: " +
    ...
    console.log(
      "members: " +
        p.members
          .slice(0, 4)
          .map(
            (m: any) => `\n  - ${m.name}, ${m.faction}, ${m.region}, ${m.temperament}, ${m.years}`,
          )
          .join(""),
    );
    console.log(
