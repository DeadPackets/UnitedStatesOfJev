...
| R27 | (2026-09-23, owner) Members: real leaders, a few iconic members, the rest invented. The faction leaders stay real. At most 10% of the seats (8 at most) may be iconic real members, each named in the sources as a member of that body on the start date and alive on it; the validator accepts a real name only on that list. Every other member and citizen is invented. Fiction worlds may seat the canon's famous figures under the same cap. |
| R28 | (2026-09-23, owner, after a blind test in docs/mocks/v4/luna-eval/RESULTS.md) The build is written by anthropic/claude-opus-5.5 at reasoning effort medium, in one request with the sources in context, with strict JSON schema output, streamed so the build log shows each section as it lands. A validator failure is fixed by a repair call for that section only, never a full redo. Luna (openai/gpt-5.6-luna) keeps every in-game call; Jev keeps every judgement. Blind scores (1 to 5, mean of look, writing, history, structure): Opus high 3.85, hand-made 3.75, Luna 2.7. |
| R29 | (2026-09-24, owner) Seat and hierarchy: the player holds the hot seat, the real office that governed day to day, unless the prompt names another. Whoever sat above it is a group with votes, a veto where real, and the new `dismiss` response (removes the ruler without a coup; its own ending). `consent` becomes `vetoes`: a list of holder ids and/or chamber or chamber_supermajority. The army holder is found by force levers, never by the coup response. Page 1 says who is above you and who can remove you. When the real holder of the seat fell inside the term, the term keeps 20 turns and the fall is the named, dated threat; its causes arrive on time. |
| R30 | (2026-09-24, owner) Faction cards: every holder and every chamber faction carries { base, wants x3 each with a yes act and a no act, redLine, price { takes, refuses }, face { name, role, line }, rival, tension }. Before signing, a vote preview shows each faction's seats as for, against and hesitant with the reason, computed from the same per-seat probabilities the real vote draws from. Negotiate: a hesitant faction offers 1 of 3 terms (a pledge with a deadline, a post, or money from the chest) for its seats; it spends clerks' time. |
| R31 | (2026-09-24, owner) Generation: a 14-way `kind` replaces `fiction` (docs: .superpowers/ruler/research/prompt-kinds.md); sources are fetched for every kind with infoboxes kept; a roster step lists every group and bloc with a verbatim source quote and grounding tag, code checks C1 to C12 (accuracy-pipeline.md) run, one repair call fixes failures, then the world call. Canon worlds use the Fandom wiki and Wikipedia; book canon wins. Absurd worlds are played straight. |
| R32 | (2026-09-24, owner) Writing: a positive style guide, a voice per world, and short exemplar passages replace the STYLE ban list, for the build and for Luna in play; code flags AI tells and sends only flagged fields back. |
| R33 | (2026-09-23 and 24, owner) Turn rules from the mock: End turn costs the public group 2 support when no act was signed that turn and gives nothing otherwise; at End turn each group an act moved is re-read and may move at most 3 more; a strike that does not end the run costs what the mock shows (riot 8 support in every region, refused levy 10 treasury, embargo 8 treasury, excommunication 25 support on your own side, a court strike voids your latest act and costs 3 authority); every crisis, foreign move and black swan card can be declined for 3 support with the group its answers would please most. |
| R34 | (2026-09-24, owner) North star: engagement, entertainment and fun first, then accuracy. Browser first (PWA), then Steam from the same code. Community worlds with ratings, accounts, a fun gate playtest, then a branding run (name, domain, landing page) before launch. |
| R35 | (2026-09-24, owner, from .superpowers/ruler/research/engagement.md) V1 engagement set: the final-vote reveal with the deciding group last; the obituary share card; a community browser (staff picks, Hot, Top by Wilson score, New, Most played; one slot in five for a new world; a like or boo counts only after a finished term, one per player per world; win rate shown as difficulty); four named levels (Landslide, Mandate, Hung, Siege; the daily is always Mandate) plus a ten-step expert ladder, with their numbers set only after the balance pass makes Mandate winnable; challenge a friend (same world, seed and level); founder credit under a display name; a publish check, reports and auto-hide; Chief of Staff assists. Leaderboards are allowed, superseding §13: a daily percentile and a top 10 per world; assisted runs are left out of both. The streak gets a grace day. |
| R36 | (2026-09-24, owner, supersedes the card fields of R30) The glance card: a file is read many times a game, so it answers in about 2 seconds what they want, what they hate and how close they are to turning. Fields: name, face (name, role, portrait), support with its line and votes, `wants` (2 to 3 short tags), `hates` (2 to 3 short tags, one flagged as the red line), and `strike` (one short line: what they do if they strike). About 20 words. The tags are also the engine's match tags, so no separate prose is generated. Dropped: base, price, rival, tension and the long wants with backs and fights. Negotiate offers a pledge tied to one of the group's wants, a post, or money unless a hate tag refuses money. Members get the same card. |
